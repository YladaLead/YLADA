...
BEGIN
  ...
  -- 2. AUMENTAR LIMITE DE META FINANCEIRA (de 20000 para 100000)
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'wellness_noel_profile' 
    ...
    ALTER TABLE wellness_noel_profile ADD CONSTRAINT wellness_noel_profile_meta_financeira_check 
      CHECK (meta_financeira >= 500 AND meta_financeira <= 100000);
      
    -- Atualizar comentário
    COMMENT ON COLUMN wellness_noel_profile.meta_financeira IS 'Meta financeira mensal (500-100000)';
  END IF;
END $$;
