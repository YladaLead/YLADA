-- =====================================================
-- INSERIR CLIENTES FICTÍCIOS - VERSÃO FINAL
-- =====================================================
-- Este script busca automaticamente seu user_id e insere os dados
-- Não precisa modificar nada, apenas execute!
-- =====================================================
...
BEGIN
  -- Buscar user_id automaticamente
  ...
  -- Verificar se o user_id existe em auth.users (onde clients referencia)
  IF v_user_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = v_user_id) THEN
      RAISE EXCEPTION 'User ID encontrado não existe em auth.users. Execute o script corrigir-constraint-clients-user-id.sql primeiro.';
    END IF;
  END IF;
  
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'Nenhum usuário encontrado. Faça login primeiro em http://localhost:3000/pt/nutri/login para criar seu perfil.';
  END IF;
