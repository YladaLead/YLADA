...
-- =====================================================
-- Garantir que cada dia tenha conteúdo único e coerente
-- Remover duplicações e garantir progressão lógica
...
SET
  ...
  action_type = 'pilar', -- ÚNICO dia que mostra Pilar 1 completo
  action_id = NULL,
...
SET
  ...
  action_type = 'exercicio', -- NÃO aponta para Pilar 1 (evita duplicação)
  action_id = NULL,
...
SET
  ...
  action_type = 'pilar', -- Aponta para Pilar 2 (Rotina)
  action_id = NULL,
...
SET
  ...
  action_type = 'exercicio', -- Não aponta para Pilar (evita duplicação)
  action_id = NULL,
...
-- VERIFICAÇÃO FINAL
-- =====================================================
-- Verificar que cada dia tem conteúdo único
SELECT 
  day_number,
  title,
  action_type,
  CASE 
    WHEN action_type = 'pilar' THEN 'Mostra conteúdo do Pilar'
...
-- Verificar se há duplicações de action_type = 'pilar'
SELECT 
  action_type,
  COUNT(*) as quantidade,
  array_agg(day_number ORDER BY day_number) as dias
FROM journey_days
WHERE day_number BETWEEN 1 AND 7
