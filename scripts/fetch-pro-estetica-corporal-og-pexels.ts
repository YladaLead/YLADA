 ...
 * Uso: pnpm og:pexels-corporal  |  npx tsx scripts/fetch-pro-estetica-corporal-og-pexels.ts
 *
 * Licença Pexels: seguir guidelines da API (atribuição quando aplicável). Relatório com URLs dos fotógrafos.
 */
...
const OUT_DIR = resolve(process.cwd(), 'public/images/og/pro-estetica-corporal')
const LISTA_PATH = resolve(process.cwd(), 'scripts/data/pro-estetica-corporal-og-pexels-queries.json')
...
async function main() {
  ...
  for (let i = 0; i < lista.arquivos.length; i++) {
    ...
    try {
      ...
      if (!photo) {
        report.push({ arquivo, ok: false, consulta: consulta_pexels, erro: 'nenhum resultado' })
        console.warn(`— ${arquivo}: sem resultados`)
        continue
      }
      const srcUrl = photo.src.large2x || photo.src.large || photo.src.original
      if (!srcUrl) {
        report.push({ arquivo, ok: false, consulta: consulta_pexels, erro: 'sem URL de imagem' })
        continue
      }
      ...
      report.push({
        ...
        consulta: consulta_pexels,
      })
      console.log(`✓ ${arquivo} ← ${photo.photographer}`)
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e)
      report.push({ arquivo, ok: false, consulta: consulta_pexels, erro: msg })
      console.error(`✗ ${arquivo}: ${msg}`)
    }
    if (i < lista.arquivos.length - 1) await sleep(400)
  }

  const reportPath = resolve(OUT_DIR, 'og-pexels-corporal-ultima-execucao.json')
  await writeFile(
    reportPath,
    JSON.stringify({ gerado_em: new Date().toISOString(), itens: report }, null, 2),
    'utf-8',
  )
  console.log(`\nRelatório (créditos Pexels): ${reportPath}`)
}
