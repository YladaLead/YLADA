-- ============================================
-- CRIAR CONTENT COMPLETO PARA QUIZ ENERGÉTICO (WELLNESS)
-- Adiciona array completo de perguntas ao content JSONB
-- ============================================

-- ⚠️ IMPORTANTE:
-- Este script atualiza o content do Quiz Energético com perguntas completas
-- Baseado no preview customizado existente

-- ============================================
-- ATUALIZAR CONTENT DO QUIZ ENERGÉTICO
-- ============================================
UPDATE templates_nutrition
SET 
  content = '{
    "template_type": "quiz",
    "profession": "wellness",
    "questions": [
      {
        "id": 1,
        "question": "Como você avalia seu nível de energia ao longo do dia?",
        "type": "multiple_choice",
        "options": [
          {"id": "a", "label": "Nunca"},
          {"id": "b", "label": "Raramente"},
          {"id": "c", "label": "Às vezes"},
          {"id": "d", "label": "Frequentemente"},
          {"id": "e", "label": "Sempre"}
        ]
      },
      {
        "id": 2,
        "question": "Você costuma sentir fadiga ou cansaço constante?",
        "type": "multiple_choice",
        "options": [
          {"id": "a", "label": "Nunca"},
          {"id": "b", "label": "Raramente"},
          {"id": "c", "label": "Às vezes"},
          {"id": "d", "label": "Frequentemente"},
          {"id": "e", "label": "Sempre"}
        ]
      },
      {
        "id": 3,
        "question": "Consome alimentos energéticos regularmente (café, chá, açúcar)?",
        "type": "multiple_choice",
        "options": [
          {"id": "a", "label": "Nunca"},
          {"id": "b", "label": "Raramente"},
          {"id": "c", "label": "Às vezes"},
          {"id": "d", "label": "Frequentemente"},
          {"id": "e", "label": "Sempre"}
        ]
      },
      {
        "id": 4,
        "question": "Nota que sua energia cai após refeições?",
        "type": "multiple_choice",
        "options": [
          {"id": "a", "label": "Nunca"},
          {"id": "b", "label": "Raramente"},
          {"id": "c", "label": "Às vezes"},
          {"id": "d", "label": "Frequentemente"},
          {"id": "e", "label": "Sempre"}
        ]
      },
      {
        "id": 5,
        "question": "Pratica atividade física regularmente para manter energia?",
        "type": "multiple_choice",
        "options": [
          {"id": "a", "label": "Nunca"},
          {"id": "b", "label": "Raramente"},
          {"id": "c", "label": "Às vezes"},
          {"id": "d", "label": "Frequentemente"},
          {"id": "e", "label": "Sempre"}
        ]
      },
      {
        "id": 6,
        "question": "Dorme bem e acorda com disposição?",
        "type": "multiple_choice",
        "options": [
          {"id": "a", "label": "Nunca"},
          {"id": "b", "label": "Raramente"},
          {"id": "c", "label": "Às vezes"},
          {"id": "d", "label": "Frequentemente"},
          {"id": "e", "label": "Sempre"}
        ]
      }
    ]
  }'::jsonb,
  updated_at = NOW()
WHERE profession = 'wellness'
  AND language = 'pt'
  AND type = 'quiz'
  AND (
    slug = 'quiz-energetico' OR
    slug = 'quiz-energético' OR
    LOWER(name) LIKE '%quiz energético%' OR
    LOWER(name) LIKE '%quiz energetico%' OR
    LOWER(name) LIKE '%energético%' OR
    LOWER(name) LIKE '%energetico%'
  );

-- ============================================
-- VERIFICAR RESULTADO
-- ============================================
SELECT 
  name, 
  slug, 
  type, 
  content->>'template_type' as template_type,
  jsonb_array_length(content->'questions') as total_perguntas,
  CASE 
    WHEN content::text LIKE '%"questions"%' AND content::text LIKE '%"question":%' THEN '✅ Content completo com perguntas'
    WHEN content::text LIKE '%"questions"%' THEN '⚠️ Content tem questions mas sem array'
    WHEN content IS NULL OR content::text = '{}' THEN '❌ Sem content'
    ELSE '⚠️ Content desconhecido'
  END as status_content
FROM templates_nutrition
WHERE profession = 'wellness'
  AND language = 'pt'
  AND type = 'quiz'
  AND (
    slug = 'quiz-energetico' OR
    slug = 'quiz-energético' OR
    LOWER(name) LIKE '%quiz energético%' OR
    LOWER(name) LIKE '%quiz energetico%' OR
    LOWER(name) LIKE '%energético%' OR
    LOWER(name) LIKE '%energetico%'
  );

