...
WITH duplicatas AS (
  SELECT slug, id, ROW_NUMBER() OVER (PARTITION BY slug ORDER BY created_at DESC, id DESC) as rn
  FROM templates_nutrition
  WHERE slug IS NOT NULL AND slug != '' AND profession = 'wellness' AND language = 'pt' AND is_active = true
)
...
UPDATE templates_nutrition
SET slug = NULL
WHERE id IN (SELECT id FROM duplicatas_finais WHERE rn > 1);
