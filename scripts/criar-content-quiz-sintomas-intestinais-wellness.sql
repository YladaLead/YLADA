-- ============================================
-- ATUALIZAR CONTENT DO QUIZ SINTOMAS INTESTINAIS (WELLNESS)
...
SET 
  content = '{
    ...
    "questions": [
      {
        "id": 1,
        "question": "Você sente desconforto digestivo, gases, inchaço ou problemas intestinais frequentemente?",
        "type": "multiple_choice",
        "options": [
          {"id": "a", "label": "Sim, tenho esses sintomas quase diariamente"},
          {"id": "b", "label": "Sim, acontece várias vezes por semana"},
          {"id": "c", "label": "Às vezes, mas não é constante"},
          {"id": "d", "label": "Raramente ou nunca tenho esses problemas"}
        ]
      },
      {
        "id": 2,
        "question": "Você sente que precisa de ajuda para melhorar sua saúde intestinal?",
        "type": "multiple_choice",
        "options": [
      ...
      {
        "id": 3,
        "question": "Você valoriza produtos que ajudam a melhorar a saúde digestiva e intestinal?",
        "type": "multiple_choice",
        "options": [
      ...
      {
        "id": 4,
        "question": "Você acredita que um plano personalizado pode transformar sua saúde intestinal?",
        "type": "multiple_choice",
        "options": [
      ...
      {
        "id": 5,
        "question": "Você está aberto(a) para ter um acompanhamento especializado em saúde digestiva?",
        "type": "multiple_choice",
        "options": [
...
WHERE profession = 'wellness'
  AND language = 'pt'
  AND type = 'quiz'
  AND (LOWER(name) LIKE '%sintoma%intestina%' OR LOWER(name) LIKE '%sintoma%intestinal%' OR LOWER(name) LIKE '%intestina%' OR slug LIKE '%sintomas-intestinais%' OR slug LIKE '%intestinais%');
