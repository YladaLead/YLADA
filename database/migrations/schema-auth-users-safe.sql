...
-- =====================================================
-- 2. ADICIONAR COLUNAS QUE PODEM FALTAR (SEM PERDER DADOS)
-- =====================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE format('ALTER TABLE %I ADD COLUMN %s', 
            p_table_name, 
            p_column_definition
        );
    END IF;
...
SELECT add_column_if_not_exists('user_profiles', 'idioma_preferido', 'VARCHAR(10) DEFAULT ''pt''');
SELECT add_column_if_not_exists('user_profiles', 'timezone', 'VARCHAR(50) DEFAULT ''America/Sao_Paulo''');
SELECT add_column_if_not_exists('user_profiles', 'created_at', 'TIMESTAMP WITH TIME ZONE DEFAULT NOW()');
SELECT add_column_if_not_exists('user_profiles', 'updated_at', 'TIMESTAMP WITH TIME ZONE DEFAULT NOW()');
...
-- Preencher email dos registros existentes que não têm email mas têm user_id
...
DO $$
BEGIN
    -- Se não houver registros NULL, tornar NOT NULL
