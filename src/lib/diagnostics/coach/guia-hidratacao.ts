/**
 * DIAGNÓSTICOS: Guia de Hidratação - ÁREA COACH
 */
...
export const guiaHidratacaoDiagnosticos: DiagnosticosPorFerramenta = {
  coach: {
    baixaHidratacao: {
      ...
      causaRaiz: '🔍 CAUSA RAIZ: Consumo insuficiente de líquidos pode afetar funções vitais, energia e desempenho físico. Estudos mostram que desidratação leve de 1-2% já pode reduzir performance cognitiva e física em até 30%. Uma avaliação de bem-estar identifica exatamente qual é sua necessidade hídrica individual e como alcançá-la gradualmente',
      acaoImediata: '⚡ AÇÃO IMEDIATA: Aumente consumo de água gradualmente, começando com 500ml a mais por dia. Busque avaliação de bem-estar para receber estratégias personalizadas de hidratação adequadas ao seu perfil, atividade e clima',
    ...
    hidratacaoModerada: {
      ...
      causaRaiz: '🔍 CAUSA RAIZ: Consumo adequado de líquidos, mas otimizações estratégicas podem elevar hidratação e resultados. Pesquisas indicam que hidratação otimizada com timing adequado pode melhorar performance em até 25% quando comparada à hidratação básica. Uma análise de bem-estar identifica exatamente quais estratégias de hidratação são mais eficazes para você',
      acaoImediata: '⚡ AÇÃO IMEDIATA: Mantenha consumo atual e otimize timing e qualidade da hidratação. Considere avaliação de bem-estar para identificar estratégias de hidratação direcionadas que potencializam seus resultados',
    ...
    altaHidratacao: {
      ...
      acaoImediata: '⚡ AÇÃO IMEDIATA: Continue padrão atual e refine estratégias de hidratação avançadas. Considere avaliação de bem-estar avançada para protocolo personalizado que maximiza resultados através de hidratação de precisão',
