...
export default function ProEsteticaCapilarOnboardingPage() {
  ...
  const validate = useCallback(async () => {
    ...
    const res = await fetch(
      `/api/pro-estetica-capilar/leader-onboarding/validate?token=${encodeURIComponent(token)}`
    )
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    setError(null)
    const res = await fetch('/api/pro-estetica-capilar/leader-onboarding/submit', {
      method: 'POST',
        ...
        {loading ? (
          ...
          <div className="space-y-3 text-center">
            ...
            <p className="text-sm text-gray-600">
              O teu micro-diagnóstico inicial já está connosco. Quando entrares no Pro Estética Capilar com este e-mail,
          ...
          <div className="space-y-4">
            <div className="space-y-2 text-center">
              <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Pro Estética Capilar</p>
            ...
            <ProEsteticaCorporalOnboardingForm
              ...
              onSubmit={onSubmit}
              saving={saving}
              serviceToGrowPlaceholder="Ex.: cronograma capilar, tratamento anti-queda, hidratação profunda…"
            />
          </div>
        ) : null}
      </div>
