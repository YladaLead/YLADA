...
export default function PortalsNutri() {
  // Renderizar diretamente sem verificação prévia - deixar a API tratar autenticação
  return <PortalsNutriContent />
}

function PortalsNutriContent() {
  ...
  const carregarUserSlug = async () => {
    try {
      ...
      // TODO: Criar API /api/nutri/profile quando necessário
  ...
  const carregarPortais = async () => {
    try {
      ...
      const response = await fetch('/api/nutri/portals', {
        credentials: 'include',
        cache: 'no-store'
      })
      ...
      if (response.status === 403) {
        ...
        setError(`Erro 403: ${errorData.error || 'Você não tem permissão para acessar esta área. Verifique seu perfil Nutri.'}`)
        setLoading(false)
        return
      }
  ...
  const deletarPortal = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/nutri/portals?id=${id}`, {
        method: 'DELETE',
        credentials: 'include'
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <NutriNavBar showTitle={true} title="Portal do Bem-Estar" />
        ...
        {error && (
          <div className="mb-6 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
            ...
            <div className="mt-3 flex space-x-3">
              <button
                onClick={() => window.location.href = '/pt/nutri/login'}
              ...
              >
                  ...
                  {carregandoSlug ? (
                    ...
                    <div className="flex items-center gap-1 flex-wrap">
                      <span className="text-xs text-gray-600 font-mono break-all">
                        {typeof window !== 'undefined' ? window.location.hostname : 'ylada.app'}/pt/nutri/<span className="text-blue-600 font-semibold">{userSlug}</span>/portal/{portal.slug}
                      </span>
                      <button
                        onClick={() => {
                          const url = `${window.location.origin}/pt/nutri/${userSlug}/portal/${portal.slug}`
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200">
                  ...
                  <div className="flex space-x-2">
                    <Link
                      href={userSlug ? `/pt/nutri/${userSlug}/portal/${portal.slug}` : `/pt/nutri/portal/${portal.slug}`}
                      target="_blank"
                    ...
                    <Link
                      href={`/pt/nutri/portals/${portal.id}/editar`}
