      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="space-y-2">
          ...
          <p className="text-gray-600 text-sm leading-relaxed">
            Quizzes, calculadoras e links prontos para usar. Filtre por segmento e tema. Em breve você poderá usar itens da biblioteca para criar novos links.
          </p>
        </header>
        ...
        {loading ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      <span className="inline-flex items-center rounded-md bg-sky-50 px-2 py-0.5 text-xs font-medium text-sky-700">
                        {BIBLIOTECA_TEMAS.find((t) => t.value === item.tema)?.label ?? item.tema}
                      </span>
                      {item.pilar && (
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (data?.success && data?.data?.id) {
                          // Calculadora: mostra modal com preview e copiar
                          // Quiz: vai para edição (título, perguntas, CTA)
                          if (item.tipo === 'calculadora' && data?.data?.url) {
                            ...
                            window.location.href = `/pt/links/editar/${data.data.id}`
                          }
                        } else if (data?.success && data?.data?.url) {
                          window.location.href = `/pt/links?created=1`
                        } else {
                          window.location.href = `/pt/links?tema=${encodeURIComponent(item.tema)}&flow_id=${item.flow_id ?? 'diagnostico_risco'}`
                        }
                      } catch {
                        window.location.href = `/pt/links?tema=${encodeURIComponent(item.tema)}&flow_id=${item.flow_id ?? 'diagnostico_risco'}`
                      } finally {
          ...
          <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/50 p-12 text-center">
            ...
            <p className="text-xs text-gray-400 mt-4">
              Filtros: {tipoAtivo} | {segmentoFiltro || 'todos'} | {temaFiltro ? BIBLIOTECA_TEMAS.find((t) => t.value === temaFiltro)?.label ?? temaFiltro : 'todos'}
            </p>
          </div>
        )}

        {/* Modal: link de calculadora criado — preview e copiar */}
        {linkCriado && (
