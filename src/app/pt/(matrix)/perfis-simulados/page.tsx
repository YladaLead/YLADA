  ...
  return (
    <YladaAreaShell areaCodigo="ylada" areaLabel="YLADA">
      <div className="max-w-2xl space-y-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900 mb-2">Perfis para testes</h1>
        ...
        {activeKey && (
          <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 flex items-center justify-between">
            <p className="text-sm text-amber-800">
              ...
              {PERFIS_SIMULADOS.find((p) => p.key === activeKey)?.label ?? activeKey}
            </p>
            <button
              type="button"
              onClick={handleDeactivate}
              className="rounded-lg px-3 py-1.5 text-sm font-medium bg-amber-200 text-amber-900 hover:bg-amber-300"
            >
              Desativar simulação
            </button>
          </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          {PERFIS_SIMULADOS.map((perfil) => {
            ...
            return (
              ...
              >
                <h2 className="font-semibold text-gray-900 mb-1">{perfil.label}</h2>
                <p className="text-xs text-gray-600 mb-3">{perfil.description}</p>
        ...
        <p className="text-xs text-gray-500">
          Vá em <strong>Noel</strong> ou <strong>Links inteligentes</strong> para testar o comportamento com o perfil ativo. Ao desativar, volta a valer seu perfil real (se tiver).
        </p>
      </div>
    </YladaAreaShell>
  )
