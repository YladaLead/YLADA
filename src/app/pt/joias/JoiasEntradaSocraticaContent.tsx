...
/**
 * Entrada pública /pt/joias — fluxo progressivo.
 * Landing minimal anterior: /pt/joiasv2.
 */
...
const CADASTRO_HREF = '/pt/cadastro?area=joias'
...
export default function JoiasEntradaSocraticaContent() {
  ...
  useEffect(() => {
    if (loading || !isJoiasRoot) return
    if (user) {
      router.replace('/pt/joias/home')
    }
  }, [loading, user, router, isJoiasRoot])
  ...
  useEffect(() => {
    if (!isJoiasRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_JOIAS_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_JOIAS_CONTINUAR_TOUR)
        setStep(JOIAS_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('joias_demo_local', { area: 'joias', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_JOIAS_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('joias_demo_nicho', { area: 'joias', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_JOIAS_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('joias_landing_escolha_inicial', { opcao: choice.analyticsValue, area: 'joias' })
      try {
        sessionStorage.setItem('ylada_joias_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
  ...
  const showAuthLoading = loading && isJoiasRoot && !authTimeout
  if (showAuthLoading) {
    return (
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Joias</p>
