...
function ModalNovaConsulta({
  ...
  // Atualizar start_time quando dataHoraPreenchida mudar
  useEffect(() => {
    if (dataHoraPreenchida?.data || dataHoraPreenchida?.hora !== undefined) {
      const novaData = dataHoraPreenchida.data || new Date()
      const novaHora = dataHoraPreenchida.hora !== undefined ? dataHoraPreenchida.hora : new Date().getHours()
      const novoMinuto = new Date().getMinutes()
      setFormData(prev => ({
        ...prev,
        start_time: formatarDataHora(novaData, novaHora, novoMinuto)
      }))
    }
  }, [dataHoraPreenchida])
