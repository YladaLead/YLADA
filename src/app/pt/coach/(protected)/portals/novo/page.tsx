...
function NovoPortalCoachContent() {
  ...
  const verificarSlug = async (slug: string) => {
    ...
    try {
      const response = await fetch(`/api/c/portals/check-slug?slug=${encodeURIComponent(slug)}`, {
        credentials: 'include'
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const portalUrl = userSlug 
        ? `${baseUrl}/pt/c/${userSlug}/portal/${portalSlug}`
        : `${baseUrl}/pt/c/portal/${portalSlug}`
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <CoachNavBar showTitle={true} title="Novo Portal de Bem-Estar" />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {carregandoSlug ? (
                  ...
                  <div className="flex flex-col space-y-2">
                    <div className="flex items-center flex-wrap gap-2">
                      <span className="text-gray-500 text-sm">ylada.app/pt/c/</span>
                      <span className="px-2 py-1 bg-purple-100 text-purple-800 rounded text-sm font-medium">{userSlug}</span>
                      ...
                      <input
                        ...
                        className="flex-1 min-w-[120px] px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                    ...
                    <p className="text-xs text-gray-500">
                      URL completa: <span className="font-mono">ylada.app/pt/c/{userSlug}/portal/{formData.slug || '[slug-do-portal]'}</span>
                    </p>
                  </div>
                )}
              </div>
                              ...
                              if (value.length >= 3) {
                                ...
                                try {
                                  const response = await fetch(
                                    `/api/c/check-short-code?code=${encodeURIComponent(value)}&type=portal`
                                  )
                                  const data = await response.json()
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            ...
            <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
              <label className="flex items-center space-x-2 mb-3">
                ...
                />
                <span className="text-sm font-medium text-purple-900">Coletar dados do cliente ao acessar o portal</span>
              </label>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            ...
            {selectedTools.length > 0 && (
              <div className="mb-4 bg-gradient-to-r from-purple-50 to-purple-50 border-2 border-purple-200 rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {selectedTools.map((toolId, index) => {
                    ...
                    return (
                      <div
                        key={toolId}
                        className="flex items-center gap-3 bg-white rounded-lg p-3 border border-purple-200 shadow-sm"
                      >
                        <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-purple-500 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md">
                          {index + 1}
                        </div>
