...
/**
 * Entrada pública /pt/coach — fluxo progressivo.
 * Landing minimal anterior: /pt/coachv2.
 */
...
import {
  ...
  STORAGE_KEY_COACH_CONTINUAR_TOUR,
...
const STEPS: StepBody[] = [
  {
    ...
    choices: [
      { label: 'Pessoas pedindo preço ou “só uma dúvida” no escuro', analyticsValue: 'preco' },
      { label: 'Conversas que não viram sessão ou programa', analyticsValue: 'conversas' },
      { label: 'Quem já chega com contexto pra primeira conversa', analyticsValue: 'prontos' },
    ],
  },
  {
    lines: [
      'Se você atua como coach…',
      '',
      'provavelmente isso já aconteceu com você:',
      '',
      'Você posta…',
      'mas quase não vira conversa de verdade',
      '',
      'Às vezes até chama gente…',
      'mas não vira compromisso',
      '',
      'E quando pede valor ou formato…',
      'a pessoa some',
      '',
      'Isso cansa.',
    ],
  ...
  },
  {
    lines: [
      'E não é você.',
      '',
      'O problema não é o seu método nem sua escuta.',
      '',
      'É que a maioria chega sem saber o que quer:',
      'objetivo, urgência, estilo de acompanhamento…',
      '',
      'E quem não sabe…',
      'compara preço.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Já com noção de foco (carreira, hábitos, negócio…)',
      'Já alinhada com expectativa de coaching',
      'E mais pronta pra sessão ou programa',
    ],
  ...
  },
  {
    lines: [
      ...
      'e pensar no que busca, no momento de vida e no próximo passo',
    ],
    primary: { label: 'Continuar', action: 'next' },
  },
  {
  ...
  },
  {
    lines: [
      ...
      '',
      '“Quero filtrar curiosos e trazer gente mais alinhada antes da primeira sessão”',
      '',
  ...
  },
  {
    lines: [
      ...
      'menos curioso pedindo “só uma call rápida” sem contexto',
      'mais clareza quando você abre o WhatsApp',
      'mais chance de sessão ou programa alinhado',
      '',
...
export default function CoachEntradaSocraticaContent() {
  ...
  const isCoachRoot = pathname === '/pt/coach' || pathname.startsWith('/pt/coach?')
  ...
  useEffect(() => {
    if (loading || !isCoachRoot) return
    if (user) {
      router.replace('/pt/coach/home')
    }
  }, [loading, user, router, isCoachRoot])
  ...
  useEffect(() => {
    if (!isCoachRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_COACH_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_COACH_CONTINUAR_TOUR)
        setStep(COACH_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('coach_demo_local', { area: 'coach', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_COACH_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('coach_demo_nicho', { area: 'coach', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_COACH_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const startDemoQuiz = useCallback(() => {
    ...
    trackEvent('coach_demo_inicio', {
      area: 'coach',
      opcao: demoLocalChoice,
      segmento: demoNichoChoice,
    })
    setDemoOpen(false)
    router.push(`${COACH_DEMO_CLIENTE_BASE_PATH}?nicho=${encodeURIComponent(demoNichoChoice)}`)
  }, [demoLocalChoice, demoNichoChoice, router])

  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('coach_landing_escolha_inicial', { opcao: choice.analyticsValue, area: 'coach' })
      try {
        sessionStorage.setItem('ylada_coach_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
  ...
  const showAuthLoading = loading && isCoachRoot && !authTimeout
  if (showAuthLoading) {
    return (
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Coach</p>
            ...
            <div className="pointer-events-auto w-full max-w-md max-h-[min(90dvh,640px)] overflow-y-auto overscroll-y-contain bg-white rounded-2xl p-6 estetica-safe-modal-bottom shadow-2xl border border-gray-200 animate-fade-in-up estetica-touch supports-[height:100svh]:max-h-[min(90svh,640px)]">
              {demoPhase === 'local' ? (
                ...
                <>
                  ...
                  <p className="mt-2 text-sm text-gray-600 leading-relaxed">
                    Fluxo curto só para demonstração. Não substitui psicoterapia nem acompanhamento clínico.
                  </p>
                  <div className="mt-5 flex flex-col gap-2">
                    {COACH_DEMO_CLIENTE_NICHOS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
