...
export default function TemplatesCoach() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.templates && data.templates.length > 0) {
            console.log('📦 Templates Coach carregados do banco:', data.templates.length)
            ...
            console.log(`✅ ${templatesFormatados.length} templates Coach formatados e carregados`)
          } else {
            console.warn('⚠️ Nenhum template Coach encontrado na API')
            setTemplates([])
          }
        } else {
          console.error('❌ Erro ao carregar templates Coach:', response.status)
          setTemplates([])
        }
      } catch (error) {
        console.error('❌ Erro ao carregar templates Coach:', error)
        setTemplates([])
      } finally {
        if (!cancelled) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <CoachNavBar showTitle title="Templates Coach" />

      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Hero */}
        <div className="bg-gradient-to-r from-purple-50 via-purple-50 to-purple-50 rounded-2xl p-8 border border-purple-100 shadow-sm mb-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-gray-700 mt-3 max-w-2xl">
                Agora a área Coach usa os mesmos templates modulados da Wellness. Escolha uma ferramenta, visualize o fluxo completo e personalize em poucos minutos mantendo o visual exclusivo do seu portal.
              </p>
            </div>
            <div className="bg-white rounded-2xl border border-purple-100 shadow p-5 w-full sm:w-auto">
              <p className="text-sm text-gray-500">Templates disponíveis</p>
              <p className="text-4xl font-bold text-purple-600">{templates.length}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6 text-sm text-gray-700">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-white shadow flex items-center justify-center text-purple-600 text-xl">✅</div>
              <div>
                <p className="font-semibold text-gray-900">{templates.length} templates modulados</p>
                <p className="text-xs text-gray-600">Com diagnósticos prontos para Coach</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-white shadow flex items-center justify-center text-purple-500 text-xl">⚡</div>
              <div>
          ...
          <div className="flex-1 min-w-[280px]">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-2 pl-12 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {templatesFiltrados.map((template) => {
              ...
              return (
                <div
                  key={template.id}
                  className="bg-white rounded-2xl border border-gray-200 hover:border-purple-300 transition-all duration-300 shadow-sm hover:shadow-lg group flex flex-col"
                >
                  <div className="p-6 flex-1">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-purple-600 text-white text-2xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                        {template.icon}
                      </div>
                  ...
                  <div className="p-6 pt-0 flex flex-col gap-3">
                    <button
                      ...
                      className="w-full bg-purple-600 text-white text-center py-2.5 rounded-xl font-semibold hover:bg-purple-700 transition-colors"
                    >
                      Ver Preview
                    </button>
                    <Link
                      href={link}
                      className="w-full text-center py-2.5 rounded-xl font-semibold border-2 border-purple-600 text-purple-700 hover:bg-purple-50 transition-colors"
                    >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={() => setTemplatePreviewAberto(null)}>
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-purple-100 text-sm">Visualize o fluxo completo deste template</p>
                  </div>
                </div>
                <button
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {(() => {
                // Debug: Log do template antes de passar para o componente
                console.log('[Coach Templates] Template selecionado para preview:', {
                  id: templatePreviewSelecionado.id,
                  nome: templatePreviewSelecionado.nome,
                  slug: templatePreviewSelecionado.slug,
                  type: templatePreviewSelecionado.type,
                  hasContent: !!templatePreviewSelecionado.content,
                  contentType: templatePreviewSelecionado.content ? typeof templatePreviewSelecionado.content : 'null',
                  contentKeys: templatePreviewSelecionado.content && typeof templatePreviewSelecionado.content === 'object' 
                    ? Object.keys(templatePreviewSelecionado.content) 
                    : [],
                  contentPreview: templatePreviewSelecionado.content 
                    ? JSON.stringify(templatePreviewSelecionado.content).substring(0, 200) 
                    : 'null'
                })
                return (
