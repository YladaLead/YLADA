...
function NovoPortalCoachContent() {
  ...
  const verificarSlug = async (slug: string) => {
    ...
    try {
      const response = await fetch(`/api/coach/portals/check-slug?slug=${encodeURIComponent(slug)}`, {
        credentials: 'include'
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const portalUrl = userSlug 
        ? `${baseUrl}/pt/coach/${userSlug}/portal/${portalSlug}`
        : `${baseUrl}/pt/coach/portal/${portalSlug}`
                ...
                {carregandoSlug ? (
                  ...
                  <div className="flex flex-col space-y-2">
                    <div className="flex items-center flex-wrap gap-2">
                      <span className="text-gray-500 text-sm">ylada.app/pt/coach/</span>
                    ...
                    <p className="text-xs text-gray-500">
                      URL completa: <span className="font-mono">ylada.app/pt/coach/{userSlug}/portal/{formData.slug || '[slug-do-portal]'}</span>
                    </p>
                  </div>
                )}
              </div>
                              ...
                              if (value.length >= 3) {
                                ...
                                try {
                                  const response = await fetch(
                                    `/api/coach/check-short-code?code=${encodeURIComponent(value)}&type=portal`
                                  )
                                  const data = await response.json()
