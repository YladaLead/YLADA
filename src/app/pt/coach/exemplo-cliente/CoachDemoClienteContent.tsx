...
/**
 * Demo: fluxo no lugar de quem busca coaching.
 * Rota: /pt/coach/exemplo-cliente?nicho=carreira_transicao
 */
...
import {
  COACH_DEMO_CLIENTE_NICHOS,
  getCoachDemoClienteConfig,
  type CoachDemoNicho,
} from '@/lib/coach-demo-cliente-data'
import { STORAGE_KEY_COACH_CONTINUAR_TOUR, STORAGE_KEY_COACH_DEMO_NICHO } from '@/lib/coach-demo-context'
import { trackEvent } from '@/lib/analytics-events'

export default function CoachDemoClienteContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const nichoParam = searchParams.get('nicho')

  const configFromUrl = useMemo(() => getCoachDemoClienteConfig(nichoParam), [nichoParam])

  const [nichoAtivo, setNichoAtivo] = useState<CoachDemoNicho | null>(configFromUrl?.value ?? null)
  const [etapa, setEtapa] = useState(0)
  const [finalizado, setFinalizado] = useState(false)

  const cfg = useMemo(() => (nichoAtivo ? getCoachDemoClienteConfig(nichoAtivo) : null), [nichoAtivo])
  ...
  const escolherNicho = useCallback(
    (v: CoachDemoNicho) => {
      try {
        sessionStorage.setItem(STORAGE_KEY_COACH_DEMO_NICHO, v)
      } catch {
        /* ok */
      }
      setNichoAtivo(v)
      setEtapa(0)
      setFinalizado(false)
      router.replace(`/pt/coach/exemplo-cliente?nicho=${v}`, { scroll: false })
    },
    [router]
  )
  ...
  const avancar = useCallback(() => {
    ...
    if (etapa < total - 1) {
      ...
      setFinalizado(true)
      trackEvent('coach_demo_cliente_concluiu', { area: 'coach', opcao: cfg.value })
    }
  ...
  const continuarTour = useCallback(() => {
    try {
      sessionStorage.setItem(STORAGE_KEY_COACH_CONTINUAR_TOUR, '1')
    } catch {
      /* ok */
    }
    router.push('/pt/coach')
  }, [router])
      ...
      <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full estetica-safe-main-bottom">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">
            Exemplo · quem busca coaching
          </p>
          <h1 className="text-xl font-bold text-gray-900 mb-2">Escolha o foco do exemplo</h1>
          <p className="text-sm text-gray-600 mb-6 leading-relaxed">
            Cada opção abre um fluxo de demonstração com perguntas reflexivas. Não substitui psicoterapia nem
            acompanhamento clínico.
          </p>
          <div className="flex flex-col gap-2">
            {COACH_DEMO_CLIENTE_NICHOS.map((n) => (
              <button
                key={n.value}
                type="button"
                onClick={() => escolherNicho(n.value as CoachDemoNicho)}
          ...
          <Link href="/pt/coach" className="mt-8 block text-center text-sm text-gray-500 hover:text-gray-800">
            ← Voltar
          </Link>
        </main>
      </div>
  ...
  if (nichoParam && !cfg) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-white">
        ...
        <Link href="/pt/coach/exemplo-cliente" className="text-blue-600 font-medium text-sm">
          Ver opções de nicho
        </Link>
      </div>
    )
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/90 p-5 mb-6">
            ...
            <p className="text-sm text-emerald-900/95 leading-relaxed mb-3">
              ...
              <span className="font-semibold">não é terapia, diagnóstico nem promessa de resultado</span>.
            </p>
            <p className="text-sm text-emerald-900/95 leading-relaxed">
              A ideia é ela <span className="font-semibold">organizar o que busca</span>, chegar com mais{' '}
          ...
          <div
            ...
            role="group"
            aria-label="Modelo do botão de WhatsApp no link do coach"
          >
            <div
          ...
          <p className="text-sm text-gray-700 leading-relaxed mb-4">
            ...
            <span className="font-semibold">contexto</span> para alinhar expectativa de coaching e reduzir troca vaga só
            de <span className="font-semibold">preço ou “o que é coaching”</span>.
          </p>
          <p className="text-sm text-gray-700 leading-relaxed mb-8">
            Isso ajuda a <span className="font-semibold">filtrar curiosos</span> e{' '}
            <span className="font-semibold">preparar</span> quem tem disposição real para o processo.
          </p>
