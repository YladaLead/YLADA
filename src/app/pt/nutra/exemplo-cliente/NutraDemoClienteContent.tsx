...
/**
 * Demo: fluxo no lugar do CLIENTE (suplementos).
 * Rota: /pt/nutra/exemplo-cliente?nicho=emagrecimento
 */
...
import {
  NUTRA_DEMO_CLIENTE_NICHOS,
  getNutraDemoClienteConfig,
  type NutraDemoNicho,
} from '@/lib/nutra-demo-cliente-data'
import { STORAGE_KEY_NUTRA_CONTINUAR_TOUR, STORAGE_KEY_NUTRA_DEMO_NICHO } from '@/lib/nutra-demo-context'
import { trackEvent } from '@/lib/analytics-events'

export default function NutraDemoClienteContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const nichoParam = searchParams.get('nicho')

  const configFromUrl = useMemo(() => getNutraDemoClienteConfig(nichoParam), [nichoParam])

  const [nichoAtivo, setNichoAtivo] = useState<NutraDemoNicho | null>(configFromUrl?.value ?? null)
  const [etapa, setEtapa] = useState(0)
  const [finalizado, setFinalizado] = useState(false)

  const cfg = useMemo(() => (nichoAtivo ? getNutraDemoClienteConfig(nichoAtivo) : null), [nichoAtivo])
  ...
  const escolherNicho = useCallback(
    (v: NutraDemoNicho) => {
      try {
        sessionStorage.setItem(STORAGE_KEY_NUTRA_DEMO_NICHO, v)
      } catch {
        /* ok */
      }
      setNichoAtivo(v)
      setEtapa(0)
      setFinalizado(false)
      router.replace(`/pt/nutra/exemplo-cliente?nicho=${v}`, { scroll: false })
    },
    [router]
  )
  ...
  const avancar = useCallback(() => {
    ...
    if (etapa < total - 1) {
      ...
      setFinalizado(true)
      trackEvent('nutra_demo_cliente_concluiu', { area: 'nutra', opcao: cfg.value })
    }
  ...
  const continuarTour = useCallback(() => {
    try {
      sessionStorage.setItem(STORAGE_KEY_NUTRA_CONTINUAR_TOUR, '1')
    } catch {
      /* ok */
    }
    router.push('/pt/nutra')
  }, [router])
      ...
      <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full estetica-safe-main-bottom">
          ...
          <p className="text-sm text-gray-600 mb-6 leading-relaxed">
            Cada opção abre um fluxo de exemplo com perguntas como as que seu cliente veria no seu link.
          </p>
          <div className="flex flex-col gap-2">
            {NUTRA_DEMO_CLIENTE_NICHOS.map((n) => (
              <button
                key={n.value}
                type="button"
                onClick={() => escolherNicho(n.value as NutraDemoNicho)}
          ...
          <Link href="/pt/nutra" className="mt-8 block text-center text-sm text-gray-500 hover:text-gray-800">
            ← Voltar
          </Link>
        </main>
      </div>
  ...
  if (nichoParam && !cfg) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-white">
        ...
        <Link href="/pt/nutra/exemplo-cliente" className="text-blue-600 font-medium text-sm">
          Ver opções de nicho
        </Link>
      </div>
    )
      ...
      <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/90 p-5 mb-6">
            <p className="text-sm font-semibold text-emerald-950 mb-2">Seu cliente recebe um diagnóstico</p>
            ...
            <p className="text-sm text-emerald-900/95 leading-relaxed">
              ...
              <span className="font-semibold">clareza</span> e costuma chegar no Zap com mais{' '}
              <span className="font-semibold">vontade de fechar</span> com orientação, em vez de só pedir preço e parar
              por aí.
            </p>
          </div>
          ...
          <div
            ...
            role="group"
            aria-label="Modelo do botão de WhatsApp no seu link Nutra"
          >
            <div
            ...
            <p className="mt-4 text-sm text-gray-700 leading-relaxed text-left">
              ...
              <span className="font-medium">seu</span> WhatsApp. A mensagem já vem alinhada ao que ele respondeu.
            </p>
          </div>
          ...
          <p className="text-sm text-gray-700 leading-relaxed mb-4">
            ...
            <span className="font-semibold">como direcionar o papo</span> e indicar com mais segurança, e o fechamento
            costuma ficar <span className="font-semibold">bem mais fácil</span>.
          </p>
          <p className="text-sm text-gray-700 leading-relaxed mb-8">
            Na prática, isso reduz aquela conversa vaga só pedindo <span className="font-semibold">“preço de tudo”</span>:
            você já entra sabendo o contexto e por onde conduzir.
          </p>
    ...
    <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
      ...
      <main className="flex-1 px-4 py-6 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
        <p className="text-xs font-semibold uppercase tracking-wider text-sky-700 bg-sky-50 border border-sky-100 rounded-lg px-3 py-2 mb-4 text-center leading-snug">
          Fluxo como seu cliente veria · {cfg.label}
        </p>
