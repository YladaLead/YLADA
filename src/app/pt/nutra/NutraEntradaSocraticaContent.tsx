...
/**
 * Entrada pública /pt/nutra — fluxo progressivo (suplementos / consultores).
 * Landing minimal anterior: /pt/nutrav2.
 */
...
import {
  ...
  STORAGE_KEY_NUTRA_CONTINUAR_TOUR,
  NUTRA_LANDING_STEP_APOS_DEMO,
} from '@/lib/nutra-demo-context'
import { NUTRA_DEMO_CLIENTE_NICHOS } from '@/lib/nutra-demo-cliente-data'

const TOTAL_STEPS = 12
const CADASTRO_HREF = '/pt/cadastro?area=nutra'
...
const STEPS: StepBody[] = [
  {
    ...
    choices: [
      { label: 'Gente pedindo preço no escuro', analyticsValue: 'preco' },
      { label: 'Conversas que não viram pedido', analyticsValue: 'conversas' },
      { label: 'Clientes prontos pra comprar com orientação', analyticsValue: 'prontos' },
    ],
  },
  {
    lines: [
      'Se você vende ou indica suplementos…',
      '',
      'provavelmente isso já aconteceu com você:',
      '',
      'Você posta…',
      'mas quase não vira pedido',
      '',
      'Às vezes até chama gente…',
      'mas não fecha com orientação',
      '',
  ...
  },
  {
    lines: [
      'E não é você.',
      '',
      'O problema não é a sua indicação com responsabilidade.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Já com mais noção de objetivo e rotina',
      'Já alinhada com a própria dúvida',
      'E mais pronta pra pedir com segurança na sua orientação',
    ],
  ...
  },
  {
    lines: [
      ...
      'e pensar no que precisa, na rotina e no próximo passo',
    ],
    primary: { label: 'Continuar', action: 'next' },
  },
  {
  ...
  },
  {
    lines: [
      ...
      '',
      '“Quero qualificar quem busca proteína pra treino”',
      '',
  ...
  },
  {
    lines: [
      ...
      'mais chance de pedido com segurança na indicação',
      '',
...
export default function NutraEntradaSocraticaContent() {
  ...
  const isNutraRoot = pathname === '/pt/nutra' || pathname.startsWith('/pt/nutra?')
  ...
  useEffect(() => {
    if (loading || !isNutraRoot) return
    if (user) {
      router.replace('/pt/nutra/home')
    }
  }, [loading, user, router, isNutraRoot])
  ...
  useEffect(() => {
    if (!isNutraRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_NUTRA_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_NUTRA_CONTINUAR_TOUR)
        setStep(NUTRA_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('nutra_demo_local', { area: 'nutra', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_NUTRA_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('nutra_demo_nicho', { area: 'nutra', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_NUTRA_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const startDemoQuiz = useCallback(() => {
    ...
    trackEvent('nutra_demo_inicio', {
      area: 'nutra',
      opcao: demoLocalChoice,
      segmento: demoNichoChoice,
    })
    setDemoOpen(false)
    router.push(`${NUTRA_DEMO_CLIENTE_BASE_PATH}?nicho=${encodeURIComponent(demoNichoChoice)}`)
  }, [demoLocalChoice, demoNichoChoice, router])

  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('nutra_landing_escolha_inicial', { opcao: choice.analyticsValue, area: 'nutra' })
      try {
        sessionStorage.setItem('ylada_nutra_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
  ...
  const showAuthLoading = loading && isNutraRoot && !authTimeout
  if (showAuthLoading) {
    return (
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Nutra · suplementos</p>
            ...
            <div className="pointer-events-auto w-full max-w-md max-h-[min(90dvh,640px)] overflow-y-auto overscroll-y-contain bg-white rounded-2xl p-6 estetica-safe-modal-bottom shadow-2xl border border-gray-200 animate-fade-in-up estetica-touch supports-[height:100svh]:max-h-[min(90svh,640px)]">
              {demoPhase === 'local' ? (
                ...
                <>
                  ...
                  <div className="mt-5 flex flex-col gap-2">
                    {NUTRA_DEMO_CLIENTE_NICHOS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
                ...
                <>
                  ...
                  <p className="mt-3 text-sm text-gray-700 leading-relaxed">
                    Responda como seu cliente responderia. O restante você vê na sequência.
                  </p>
