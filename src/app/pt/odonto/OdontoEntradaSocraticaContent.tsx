...
/**
 * Entrada pública /pt/odonto — fluxo progressivo (espelho nutri/estética, copy odonto).
 * Landing minimal anterior: /pt/odontov2.
 */
...
import {
  ...
  STORAGE_KEY_ODONTO_CONTINUAR_TOUR,
  ODONTO_LANDING_STEP_APOS_DEMO,
...
const CADASTRO_HREF = '/pt/cadastro?area=odonto'
...
const STEPS: StepBody[] = [
  {
    ...
    choices: [
      { label: 'Pessoas pedindo preço no escuro', analyticsValue: 'preco' },
      { label: 'Conversas que não viram avaliação ou retorno', analyticsValue: 'conversas' },
      { label: 'Pacientes com intenção clara de tratamento', analyticsValue: 'prontos' },
    ],
  },
  {
    lines: [
      'Se você atende em odontologia…',
      '',
      'provavelmente isso já aconteceu com você:',
      '',
      'Você posta…',
      'mas quase não vira paciente na agenda',
      '',
      'Às vezes até chama gente…',
      'mas não vira avaliação ou retorno',
      '',
      'E quando pergunta preço…',
      'a pessoa some',
      '',
      'Isso cansa.',
    ],
  ...
  },
  {
    lines: [
      'E não é você.',
      '',
      'O problema não é o seu trabalho na cadeira.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Já com mais noção do que sente na boca',
      'Já alinhada com urgência e expectativa',
      'E mais pronta pra avaliação ou plano',
    ],
  ...
  },
  {
    lines: [
      ...
      'e pensar na dor, na prioridade e no que espera do tratamento',
    ],
    primary: { label: 'Continuar', action: 'next' },
  },
  {
  ...
  },
  {
    lines: [
      ...
      '',
      '“Quero atrair pacientes para avaliação de implante”',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'ela vai clareando o que precisa',
      '',
  ...
  },
  {
    lines: [
      ...
      'menos curioso pedindo preço sem contexto',
      'mais clareza quando você abre o WhatsApp',
      'mais chance de avaliação ou retorno',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Precisa fazer o paciente chegar melhor.',
      '',
...
export default function OdontoEntradaSocraticaContent() {
  ...
  const isOdontoRoot = pathname === '/pt/odonto' || pathname.startsWith('/pt/odonto?')
  ...
  useEffect(() => {
    if (loading || !isOdontoRoot) return
    if (user) {
      router.replace('/pt/odonto/home')
    }
  }, [loading, user, router, isOdontoRoot])
  ...
  useEffect(() => {
    if (!isOdontoRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_ODONTO_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_ODONTO_CONTINUAR_TOUR)
        setStep(ODONTO_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('odonto_demo_local', { area: 'odonto', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_ODONTO_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('odonto_demo_nicho', { area: 'odonto', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_ODONTO_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const startDemoQuiz = useCallback(() => {
    ...
    trackEvent('odonto_demo_inicio', {
      area: 'odonto',
      opcao: demoLocalChoice,
      segmento: demoNichoChoice,
    })
    setDemoOpen(false)
    router.push(`${ODONTO_DEMO_CLIENTE_BASE_PATH}?nicho=${encodeURIComponent(demoNichoChoice)}`)
  }, [demoLocalChoice, demoNichoChoice, router])

  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('odonto_landing_escolha_inicial', { opcao: choice.analyticsValue, area: 'odonto' })
      try {
        sessionStorage.setItem('ylada_odonto_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
  ...
  const showAuthLoading = loading && isOdontoRoot && !authTimeout
  if (showAuthLoading) {
    return (
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Odontologia</p>
            ...
            <div className="pointer-events-auto w-full max-w-md max-h-[min(90dvh,640px)] overflow-y-auto overscroll-y-contain bg-white rounded-2xl p-6 estetica-safe-modal-bottom shadow-2xl border border-gray-200 animate-fade-in-up estetica-touch supports-[height:100svh]:max-h-[min(90svh,640px)]">
              {demoPhase === 'local' ? (
                ...
                <>
                  ...
                  <div className="mt-5 flex flex-col gap-2">
                    {ODONTO_DEMO_CLIENTE_NICHOS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
                ...
                <EntradaSocraticaDemoIntroPanel
                  quemRespondeNoExemplo="seu paciente"
