...
/**
 * Página de medicina: mesma estrutura e seções da home YLADA (/pt) e da estética,
 * com conteúdo 100% focado em médicos (argumentos, fluxo, CTAs).
 * Dor principal: agenda instável. Fluxo sem perguntar área; vídeo com initialArea="med".
 */
...
const EXEMPLOS_MED = [
  { titulo: 'Seus sintomas podem estar relacionados ao estresse?', href: '/pt/diagnostico?area=0' },
  { titulo: 'O que está impedindo sua qualidade de vida?', href: '/pt/diagnostico?area=0' },
  { titulo: 'Vale a pena agendar uma avaliação?', href: '/pt/diagnostico?area=0' },
  { titulo: 'Você está pronto para uma consulta com contexto?', href: '/pt/diagnostico?area=0' },
]

const HOW_IT_WORKS_MED = [
  { title: 'Criar diagnóstico', desc: 'Transforme seu conhecimento em perguntas que revelam sintomas e necessidades do paciente.' },
...
export default function MedInstitutionalContent() {
  const pathname = usePathname() ?? ''
  const locale = pathname.startsWith('/en') ? 'en' : pathname.startsWith('/es') ? 'es' : 'pt'
  const { user, loading } = useAuth()
  const router = useRouter()

  const isMedPage = /^\/pt\/med\/?(\?|$)/.test(pathname)
  ...
  useEffect(() => {
    if (loading || !isMedPage) return
    if (user) {
      router.replace('/pt/med/home')
    }
  }, [loading, user, pathname, router, isMedPage])

  const showAuthLoading = loading && isMedPage && !authTimeout
  if (showAuthLoading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-50 border-b border-gray-200 bg-white shadow-sm h-[72px] flex items-center safe-area-inset-top">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-full">
          <Link href="/pt" className="flex items-center gap-2 flex-shrink-0 touch-manipulation" aria-label="YLADA início">
            ...
            <span className="text-gray-500 text-sm font-medium hidden sm:inline">· Medicina</span>
          </Link>
          <nav className="flex items-center gap-3 sm:gap-6">
            <Link href="/pt/diagnostico?area=0" className="text-gray-600 hover:text-gray-900 text-sm font-medium hidden md:inline">
              Fazer diagnóstico
            </Link>
            ...
            <Link href="/pt/med/login" className="text-gray-600 hover:text-gray-900 text-sm font-medium">
              Entrar
            </Link>
            <Link
              href="/pt/med/login"
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (respostaHeroIdx !== null) {
                        ...
                        router.push(`/pt/diagnostico?fromHome=1&area=0&problema=${problemaValue}`)
                      }
                    }}
        ...
        <DemoCarouselYLADA initialArea="med" />
        ...
        <section className="bg-gray-50 py-12 sm:py-16 lg:py-20 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            <h2 className="text-xl sm:text-3xl font-bold text-gray-900 mb-6 sm:mb-8 text-center">
              A maioria dos médicos explica demais antes do primeiro contato.
            </h2>
            <ul className="space-y-3 text-gray-700 mb-6">
              <li className="flex items-start gap-3">
                <span className="text-red-500 mt-1">•</span>
                <span>Agenda que não preenche ou que enche de curiosos</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-red-500 mt-1">•</span>
              ...
              <li className="flex items-start gap-3">
                <span className="text-red-500 mt-1">•</span>
                <span>Pacientes que não entendem o valor da avaliação</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-red-500 mt-1">•</span>
            ...
            <p className="text-lg font-semibold text-gray-900 text-center mb-8">
              ...
              É a forma como o paciente chega até você — e por que a agenda não estabiliza.
            </p>
        ...
        <section className="bg-gray-50 py-10 sm:py-14 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-xl mx-auto text-center">
            ...
            <p className="text-gray-700 mb-6 leading-relaxed">
              Quando uma pessoa responde perguntas sobre sintomas, preocupações ou qualidade de vida, ela começa a entender a necessidade antes da conversa.
            </p>
        ...
        <section className="py-12 sm:py-16 lg:py-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-xl sm:text-3xl font-bold text-gray-900 text-center mb-4">
              Exemplos de diagnósticos para médicos
            </h2>
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              {EXEMPLOS_MED.map((ex, i) => (
                <Link
                  key={i}
                  href={ex.href}
            ...
            <p className="text-center text-gray-600 text-sm">
              Esses são exemplos de diagnósticos que médicos podem criar com YLADA.
            </p>
          </div>
        </section>
        ...
        <section className="bg-gray-50 py-12 sm:py-16 lg:py-20 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6">
              Médicos usam diagnósticos para:
            </h2>
        ...
        <YladaProfissionaisGridSection locale={locale} highlightCodigo="med" />
        ...
        <div className="w-full max-w-7xl mx-auto px-4 py-8 sm:py-10">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <nav className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 mb-6 text-sm">
              <Link href="/pt/diagnostico?area=0" className="text-gray-600 hover:text-gray-900">Fazer diagnóstico</Link>
              ...
              <Link href="/pt/med/login" className="text-gray-600 hover:text-gray-900">Entrar</Link>
              <Link href="/pt/med/login" className="text-blue-600 hover:text-blue-700 font-medium">Criar diagnóstico</Link>
            </nav>
