...
/**
 * Entrada pública /pt/med — fluxo progressivo (medicina).
...
import {
  ...
  STORAGE_KEY_MED_CONTINUAR_TOUR,
  MED_LANDING_STEP_APOS_DEMO,
} from '@/lib/med-demo-context'
import { MED_DEMO_CLIENTE_NICHOS } from '@/lib/med-demo-cliente-data'

const TOTAL_STEPS = 12
const CADASTRO_HREF = '/pt/cadastro?area=med'
...
const STEPS: StepBody[] = [
  {
    ...
    choices: [
      { label: 'Pessoas pedindo preço ou consulta no escuro', analyticsValue: 'preco' },
      { label: 'Conversas que não viram consulta ou retorno', analyticsValue: 'conversas' },
      { label: 'Pacientes com contexto claro pra primeira conversa', analyticsValue: 'prontos' },
    ],
  },
  {
    lines: [
      'Se você atende como médico ou médica…',
      '',
      ...
      'mas não vira consulta ou retorno',
      '',
      'E quando pergunta preço ou convênio…',
      'a pessoa some',
      '',
      'Isso cansa.',
    ],
  ...
  },
  {
    lines: [
      'E não é você.',
      '',
      'O problema não é o seu trabalho na consulta.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Já com mais noção da queixa e do tempo dos sintomas',
      'Já alinhada com expectativa e urgência percebida',
      'E mais pronta pra consulta ou retorno com contexto',
    ],
  ...
  },
  {
    lines: [
      ...
      'e pensar na queixa, na rotina e no que espera da consulta',
    ],
    primary: { label: 'Continuar', action: 'next' },
  },
  {
  ...
  },
  {
    lines: [
      ...
      '',
      '“Quero atrair conversas para consulta de rotina e dúvidas antes do agendamento”',
      '',
  ...
  },
  {
    lines: [
      ...
      'mais chance de consulta ou retorno',
      '',
...
export default function MedEntradaSocraticaContent() {
  ...
  useEffect(() => {
    if (loading || !isMedRoot) return
    if (user) {
      router.replace('/pt/med/home')
    }
  }, [loading, user, router, isMedRoot])
  ...
  useEffect(() => {
    if (!isMedRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_MED_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_MED_CONTINUAR_TOUR)
        setStep(MED_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('med_demo_local', { area: 'med', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_MED_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('med_demo_nicho', { area: 'med', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_MED_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const startDemoQuiz = useCallback(() => {
    ...
    trackEvent('med_demo_inicio', {
      area: 'med',
      opcao: demoLocalChoice,
      segmento: demoNichoChoice,
    })
    setDemoOpen(false)
    router.push(`${MED_DEMO_CLIENTE_BASE_PATH}?nicho=${encodeURIComponent(demoNichoChoice)}`)
  }, [demoLocalChoice, demoNichoChoice, router])

  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('med_landing_escolha_inicial', { opcao: choice.analyticsValue, area: 'med' })
      try {
        sessionStorage.setItem('ylada_med_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
  ...
  const showAuthLoading = loading && isMedRoot && !authTimeout
  if (showAuthLoading) {
    return (
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Medicina</p>
            ...
            <div className="pointer-events-auto w-full max-w-md max-h-[min(90dvh,640px)] overflow-y-auto overscroll-y-contain bg-white rounded-2xl p-6 estetica-safe-modal-bottom shadow-2xl border border-gray-200 animate-fade-in-up estetica-touch supports-[height:100svh]:max-h-[min(90svh,640px)]">
              {demoPhase === 'local' ? (
                ...
                <>
                  ...
                  <p className="mt-2 text-sm text-gray-600 leading-relaxed">
                    Fluxo curto só para demonstração. Não é triagem clínica nem emergência.
                  </p>
                  <div className="mt-5 flex flex-col gap-2">
                    {MED_DEMO_CLIENTE_NICHOS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
                ...
                <>
                  ...
                  <p className="mt-3 text-sm text-gray-700 leading-relaxed">
                    Responda como o paciente responderia. O restante você vê na sequência.
                  </p>
