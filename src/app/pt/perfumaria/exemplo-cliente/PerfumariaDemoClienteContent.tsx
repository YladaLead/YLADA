...
/**
 * Demo: fluxo no lugar do cliente (perfumaria).
 * Rota: /pt/perfumaria/exemplo-cliente?nicho=primeira_fragancia
 */
...
import {
  PERFUMARIA_DEMO_CLIENTE_NICHOS,
  getPerfumariaDemoClienteConfig,
  type PerfumariaDemoNicho,
} from '@/lib/perfumaria-demo-cliente-data'
import {
...
export default function PerfumariaDemoClienteContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const nichoParam = searchParams.get('nicho')

  const configFromUrl = useMemo(() => getPerfumariaDemoClienteConfig(nichoParam), [nichoParam])

  const [nichoAtivo, setNichoAtivo] = useState<PerfumariaDemoNicho | null>(configFromUrl?.value ?? null)
  const [etapa, setEtapa] = useState(0)
  const [finalizado, setFinalizado] = useState(false)

  const cfg = useMemo(
    () => (nichoAtivo ? getPerfumariaDemoClienteConfig(nichoAtivo) : null),
    [nichoAtivo]
  )
  ...
  const escolherNicho = useCallback(
    (v: PerfumariaDemoNicho) => {
      try {
        sessionStorage.setItem(STORAGE_KEY_PERFUMARIA_DEMO_NICHO, v)
      } catch {
        /* ok */
      }
      setNichoAtivo(v)
      setEtapa(0)
      setFinalizado(false)
      router.replace(`/pt/perfumaria/exemplo-cliente?nicho=${v}`, { scroll: false })
    },
    [router]
  )
  ...
  const avancar = useCallback(() => {
    ...
    if (etapa < total - 1) {
      ...
      setFinalizado(true)
      trackEvent('perfumaria_demo_cliente_concluiu', { area: 'perfumaria', opcao: cfg.value })
    }
  ...
  const continuarTour = useCallback(() => {
    try {
      sessionStorage.setItem(STORAGE_KEY_PERFUMARIA_CONTINUAR_TOUR, '1')
    } catch {
      /* ok */
    }
    router.push('/pt/perfumaria')
  }, [router])
      ...
      <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full estetica-safe-main-bottom">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">
            Exemplo · visão do cliente
          </p>
          <h1 className="text-xl font-bold text-gray-900 mb-2">Escolha o foco do exemplo</h1>
          <p className="text-sm text-gray-600 mb-6 leading-relaxed">
            Cada opção abre um fluxo de demonstração com perguntas reflexivas. Não substitui prova na pele nem
            avaliação de alergia.
          </p>
          <div className="flex flex-col gap-2">
            {PERFUMARIA_DEMO_CLIENTE_NICHOS.map((n) => (
              <button
                key={n.value}
                type="button"
                onClick={() => escolherNicho(n.value as PerfumariaDemoNicho)}
          ...
          <Link href="/pt/perfumaria" className="mt-8 block text-center text-sm text-gray-500 hover:text-gray-800">
            ← Voltar
          </Link>
        </main>
      </div>
  ...
  if (nichoParam && !cfg) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-white">
        ...
        <Link href="/pt/perfumaria/exemplo-cliente" className="text-blue-600 font-medium text-sm">
          Ver opções de nicho
        </Link>
      </div>
    )
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/90 p-5 mb-6">
            ...
            <p className="text-sm text-emerald-900/95 leading-relaxed mb-3">
              ...
              <span className="font-semibold">não substitui prova na pele, amostra nem orientação individual de saúde</span>.
            </p>
            <p className="text-sm text-emerald-900/95 leading-relaxed">
              A ideia é ela <span className="font-semibold">organizar preferências</span>, chegar com mais{' '}
          ...
          <div
            ...
            role="group"
            aria-label="Modelo do botão de WhatsApp no link de perfumaria"
          >
            <div
          ...
          <p className="text-sm text-gray-700 leading-relaxed mb-4">
            ...
            <span className="font-semibold">contexto</span> para indicar com segurança e reduzir troca vaga só de{' '}
            <span className="font-semibold">preço ou print da concorrência</span>.
          </p>
          <p className="text-sm text-gray-700 leading-relaxed mb-8">
            Isso ajuda a <span className="font-semibold">filtrar curiosos</span> e{' '}
            <span className="font-semibold">preparar</span> quem realmente quer comprar ou ser acompanhada na escolha.
          </p>
    ...
    <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
      ...
      <main className="flex-1 px-4 py-6 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
        <p className="text-xs font-semibold uppercase tracking-wider text-sky-700 bg-sky-50 border border-sky-100 rounded-lg px-3 py-2 mb-4 text-center leading-snug">
          Fluxo como o cliente veria · {cfg.label}
        </p>
