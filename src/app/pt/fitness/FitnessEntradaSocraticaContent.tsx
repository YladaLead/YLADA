...
/**
 * Entrada pública /pt/fitness — fluxo progressivo.
 * Landing minimal anterior: /pt/fitnessv2.
 */
...
const STEPS: StepBody[] = [
  {
    ...
    choices: [
      { label: 'Gente pedindo preço ou “só uma dúvida” sem contexto', analyticsValue: 'preco' },
      { label: 'Conversas que não viram aula ou plano', analyticsValue: 'conversas' },
      { label: 'Quem já chega com objetivo e rotina mais claros', analyticsValue: 'prontos' },
    ],
  },
  {
    lines: [
      'Se você trabalha com treino, performance ou estúdio…',
      '',
      'provavelmente isso já aconteceu com você:',
      '',
      'Você posta…',
      'mas quase não vira aluno de verdade',
      '',
      'Às vezes até chama gente…',
      'mas não vira compromisso',
      '',
      'E quando pede valor ou frequência…',
      'a pessoa some',
      '',
      'Isso cansa.',
    ],
  ...
  },
  {
    lines: [
      'E não é você.',
      '',
      'O problema não é seu método nem seu carisma.',
      '',
      'É que a maioria chega sem saber o que quer:',
      'objetivo, tempo disponível, limitações…',
      '',
      'E quem não sabe…',
      'compara preço.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Já com noção de objetivo (emagrecer, força, condicionamento…)',
      'Já alinhada com formato (online, presencial, grupo)',
      'E mais pronta pra aula ou plano',
    ],
  ...
  },
  {
    lines: [
      ...
      'e pensar em rotina, experiência e próximo passo',
    ],
    primary: { label: 'Continuar', action: 'next' },
  },
  {
  ...
  },
  {
    lines: [
      ...
      '',
      '“Quero filtrar curiosos e trazer gente mais alinhada antes da primeira aula”',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'E começa a enxergar prioridades que antes não nomeava.',
      '',
  ...
  },
  {
    lines: [
      ...
      'menos curioso pedindo “só uma planilha” no escuro',
      'mais clareza quando você abre o WhatsApp',
      'mais chance de aula experimental ou plano alinhado',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Precisa fazer a pessoa chegar melhor.',
      '',
...
export default function FitnessEntradaSocraticaContent() {
  ...
  const isFitnessRoot = pathname === '/pt/fitness' || pathname.startsWith('/pt/fitness?')
  ...
  useEffect(() => {
    if (loading || !isFitnessRoot) return
    if (user) {
      router.replace('/pt/fitness/home')
    }
  }, [loading, user, router, isFitnessRoot])
  ...
  useEffect(() => {
    if (!isFitnessRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_FITNESS_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_FITNESS_CONTINUAR_TOUR)
        setStep(FITNESS_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('fitness_demo_local', { area: 'fitness', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_FITNESS_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('fitness_demo_nicho', { area: 'fitness', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_FITNESS_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const startDemoQuiz = useCallback(() => {
    ...
    trackEvent('fitness_demo_inicio', {
      area: 'fitness',
      opcao: demoLocalChoice,
      segmento: demoNichoChoice,
    })
    setDemoOpen(false)
    router.push(`${FITNESS_DEMO_CLIENTE_BASE_PATH}?nicho=${encodeURIComponent(demoNichoChoice)}`)
  }, [demoLocalChoice, demoNichoChoice, router])

  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('fitness_landing_escolha_inicial', { opcao: choice.analyticsValue, area: 'fitness' })
      try {
        sessionStorage.setItem('ylada_fitness_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
  ...
  const showAuthLoading = loading && isFitnessRoot && !authTimeout
  if (showAuthLoading) {
    return (
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Fitness</p>
            ...
            <div className="pointer-events-auto w-full max-w-md max-h-[min(90dvh,640px)] overflow-y-auto overscroll-y-contain bg-white rounded-2xl p-6 estetica-safe-modal-bottom shadow-2xl border border-gray-200 animate-fade-in-up estetica-touch supports-[height:100svh]:max-h-[min(90svh,640px)]">
              {demoPhase === 'local' ? (
                ...
                <>
                  ...
                  <div className="mt-5 flex flex-col gap-2">
                    {FITNESS_DEMO_CLIENTE_NICHOS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
