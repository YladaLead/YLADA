...
/**
 * Página de psicologia: mesma estrutura e seções da home e da estética,
 * com conteúdo 100% focado em psicólogos (dor conversas não evoluem, fluxo sem perguntar área, vídeo psi).
 */
...
const EXEMPLOS_PSI = [
  { titulo: 'Seu nível de ansiedade pode estar alto?', href: '/pt/diagnostico?area=1' },
  { titulo: 'Entenda o que você precisa agora', href: '/pt/diagnostico?area=1' },
  { titulo: 'Você sente dificuldade emocional?', href: '/pt/diagnostico?area=1' },
  { titulo: 'Você está pronto para uma conversa com contexto?', href: '/pt/diagnostico?area=1' },
]

const HOW_IT_WORKS_PSI = [
  { title: 'Criar diagnóstico', desc: 'Transforme seu conhecimento em perguntas que revelam necessidades e o momento da pessoa.' },
  ...
  { title: 'Conversa com contexto', desc: 'O cliente chega com mais clareza e a conversa evolui com mais facilidade.' },
]

export default function PsiInstitutionalContent() {
  const pathname = usePathname() ?? ''
  const locale = pathname.startsWith('/en') ? 'en' : pathname.startsWith('/es') ? 'es' : 'pt'
  const { user, loading } = useAuth()
  const router = useRouter()

  const isPsiPage = /^\/pt\/psi\/?(\?|$)/.test(pathname)
  ...
  useEffect(() => {
    if (loading || !isPsiPage) return
    if (user) {
      router.replace('/pt/psi/home')
    }
  }, [loading, user, pathname, router, isPsiPage])

  const showAuthLoading = loading && isPsiPage && !authTimeout
  if (showAuthLoading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-50 border-b border-gray-200 bg-white shadow-sm h-[72px] flex items-center safe-area-inset-top">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-full">
          <Link href="/pt" className="flex items-center gap-2 flex-shrink-0 touch-manipulation" aria-label="YLADA início">
            ...
            <span className="text-gray-500 text-sm font-medium hidden sm:inline">· Psicologia</span>
          </Link>
          <nav className="flex items-center gap-3 sm:gap-6">
            <Link href="/pt/diagnostico?area=1" className="text-gray-600 hover:text-gray-900 text-sm font-medium hidden md:inline">
              Fazer diagnóstico
            </Link>
            ...
            <Link href="/pt/psi/login" className="text-gray-600 hover:text-gray-900 text-sm font-medium">
              Entrar
            </Link>
            <Link
              href="/pt/psi/login"
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (respostaHeroIdx !== null) {
                        ...
                        router.push(`/pt/diagnostico?fromHome=1&area=1&problema=${problemaValue}`)
                      }
                    }}
        ...
        <DemoCarouselYLADA initialArea="psi" />
        ...
        <section className="bg-gray-50 py-12 sm:py-16 lg:py-20 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            <h2 className="text-xl sm:text-3xl font-bold text-gray-900 mb-6 sm:mb-8 text-center">
              A maioria dos psicólogos explica demais antes da primeira conversa.
            </h2>
        ...
        <section className="py-12 sm:py-16 lg:py-20">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-xl sm:text-3xl font-bold text-gray-900 text-center mb-4">
              Exemplos de diagnósticos para psicólogos
            </h2>
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              {EXEMPLOS_PSI.map((ex, i) => (
                <Link
                  key={i}
                  href={ex.href}
            ...
            <p className="text-center text-gray-600 text-sm">
              Esses são exemplos de diagnósticos que psicólogos podem criar com YLADA.
            </p>
          </div>
        </section>
        ...
        <section className="bg-gray-50 py-12 sm:py-16 lg:py-20 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6">
              Psicólogos usam diagnósticos para:
            </h2>
            <ul className="space-y-2 text-gray-700">
              <li className="flex items-center justify-center gap-2">
                <span className="text-green-600">✔</span>
                <span>atrair clientes mais preparados para conversar</span>
              </li>
              <li className="flex items-center justify-center gap-2">
                <span className="text-green-600">✔</span>
        ...
        <YladaProfissionaisGridSection locale={locale} highlightCodigo="psi" />
        ...
        <div className="w-full max-w-7xl mx-auto px-4 py-8 sm:py-10">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <nav className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 mb-6 text-sm">
              <Link href="/pt/diagnostico?area=1" className="text-gray-600 hover:text-gray-900">Fazer diagnóstico</Link>
