...
/**
 * Demo: fluxo no lugar de quem busca psicologia.
 * Rota: /pt/psi/exemplo-cliente?nicho=ansiedade_rotina
 */
...
import {
  PSI_DEMO_CLIENTE_NICHOS,
  getPsiDemoClienteConfig,
  type PsiDemoNicho,
} from '@/lib/psi-demo-cliente-data'
import { STORAGE_KEY_PSI_CONTINUAR_TOUR, STORAGE_KEY_PSI_DEMO_NICHO } from '@/lib/psi-demo-context'
import { trackEvent } from '@/lib/analytics-events'

export default function PsiDemoClienteContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const nichoParam = searchParams.get('nicho')

  const configFromUrl = useMemo(() => getPsiDemoClienteConfig(nichoParam), [nichoParam])

  const [nichoAtivo, setNichoAtivo] = useState<PsiDemoNicho | null>(configFromUrl?.value ?? null)
  const [etapa, setEtapa] = useState(0)
  const [finalizado, setFinalizado] = useState(false)

  const cfg = useMemo(() => (nichoAtivo ? getPsiDemoClienteConfig(nichoAtivo) : null), [nichoAtivo])
  ...
  const escolherNicho = useCallback(
    (v: PsiDemoNicho) => {
      try {
        sessionStorage.setItem(STORAGE_KEY_PSI_DEMO_NICHO, v)
      } catch {
        /* ok */
      }
      setNichoAtivo(v)
      setEtapa(0)
      setFinalizado(false)
      router.replace(`/pt/psi/exemplo-cliente?nicho=${v}`, { scroll: false })
    },
    [router]
  )
  ...
  const avancar = useCallback(() => {
    ...
    if (etapa < total - 1) {
      ...
      setFinalizado(true)
      trackEvent('psi_demo_cliente_concluiu', { area: 'psi', opcao: cfg.value })
    }
  ...
  const continuarTour = useCallback(() => {
    try {
      sessionStorage.setItem(STORAGE_KEY_PSI_CONTINUAR_TOUR, '1')
    } catch {
      /* ok */
    }
    router.push('/pt/psi')
    ...
    return (
      <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full estetica-safe-main-bottom">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Exemplo · quem busca psicologia</p>
          <h1 className="text-xl font-bold text-gray-900 mb-2">Escolha o foco do exemplo</h1>
          <p className="text-sm text-gray-600 mb-6 leading-relaxed">
            Cada opção abre um fluxo de demonstração com perguntas reflexivas, como alguém veria no seu link antes de falar com você.
          </p>
          <div className="flex flex-col gap-2">
            {PSI_DEMO_CLIENTE_NICHOS.map((n) => (
              <button
                key={n.value}
                type="button"
                onClick={() => escolherNicho(n.value as PsiDemoNicho)}
          ...
          <Link href="/pt/psi" className="mt-8 block text-center text-sm text-gray-500 hover:text-gray-800">
            ← Voltar
          </Link>
        </main>
      </div>
  ...
  if (nichoParam && !cfg) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-white">
        ...
        <Link href="/pt/psi/exemplo-cliente" className="text-blue-600 font-medium text-sm">
          Ver opções de nicho
        </Link>
      </div>
    )
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/90 p-5 mb-6">
            ...
            <p className="text-sm text-emerald-900/95 leading-relaxed mb-3">
              Na sessão real, o conteúdo é ligado ao que ela respondeu. Aqui é só demonstração:{' '}
              <span className="font-semibold">não é diagnóstico nem avaliação clínica</span>.
            </p>
            <p className="text-sm text-emerald-900/95 leading-relaxed">
              A ideia é ela <span className="font-semibold">se sentir mais compreendida</span>, ganhar{' '}
              <span className="font-semibold">clareza</span> para a primeira conversa e chegar no Zap com mais contexto, em
              vez de só pedir preço e parar.
            </p>
          </div>
          ...
          <div
            ...
            role="group"
            aria-label="Modelo do botão de WhatsApp no link da psicologia"
          >
            <div
          ...
          <p className="text-sm text-gray-700 leading-relaxed mb-4">
            ...
            <span className="font-semibold">como acolher e direcionar</span>, e o convite pra primeira sessão costuma ficar{' '}
            <span className="font-semibold">mais alinhado</span>.
          </p>
          <p className="text-sm text-gray-700 leading-relaxed mb-8">
            Na prática, isso reduz aquela troca vaga só pedindo <span className="font-semibold">“quanto custa a sessão”</span>{' '}
            sem contexto: você já entra sabendo por onde começar o acolhimento.
          </p>
