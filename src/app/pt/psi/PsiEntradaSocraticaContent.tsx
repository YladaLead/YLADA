...
/**
 * Entrada pública /pt/psi — fluxo progressivo (psicologia).
...
import {
  ...
  STORAGE_KEY_PSI_CONTINUAR_TOUR,
  PSI_LANDING_STEP_APOS_DEMO,
...
const CADASTRO_HREF = '/pt/cadastro?area=psi'
...
const STEPS: StepBody[] = [
  {
    ...
    choices: [
      { label: 'Pessoas perguntando preço antes de contexto', analyticsValue: 'preco' },
      { label: 'Conversas que não viram sessão ou retorno', analyticsValue: 'conversas' },
      { label: 'Pessoas com intenção clara de começar um processo', analyticsValue: 'prontos' },
    ],
  },
  {
    lines: [
      'Se você atende como psicólogo ou psicóloga…',
      '',
      'provavelmente isso já aconteceu com você:',
      '',
      'Você posta…',
      'mas quase não vira primeira sessão',
      '',
      'Às vezes até chama gente…',
      'mas não vira retorno ou continuidade',
      '',
      'E quando pergunta preço ou disponibilidade…',
      'a pessoa some',
      '',
      'Isso cansa.',
    ],
  ...
  },
  {
    lines: [
      'E não é você.',
      '',
      'O problema não é a sua escuta clínica.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Já com mais noção do que incomoda',
      'Já com contexto e expectativa mais claros',
      'E mais aberta pra vínculo e continuidade',
    ],
  ...
  },
  {
    lines: [
      ...
      'sobre o que sente, o contexto e o que busca no processo',
    ],
    primary: { label: 'Continuar', action: 'next' },
  },
  {
  ...
  },
  {
    lines: [
      ...
      '',
      '“Quero atrair primeiras conversas sobre ansiedade e rotina”',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'ela vai clareando o que sente e o que precisa',
      '',
  ...
  },
  {
    lines: [
      ...
      'mais chance de primeira sessão com alinhamento',
      '',
...
export default function PsiEntradaSocraticaContent() {
  ...
  useEffect(() => {
    if (loading || !isPsiRoot) return
    if (user) {
      router.replace('/pt/psi/home')
    }
  }, [loading, user, router, isPsiRoot])
  ...
  useEffect(() => {
    if (!isPsiRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_PSI_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_PSI_CONTINUAR_TOUR)
        setStep(PSI_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('psi_demo_local', { area: 'psi', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_PSI_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('psi_demo_nicho', { area: 'psi', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_PSI_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const startDemoQuiz = useCallback(() => {
    ...
    trackEvent('psi_demo_inicio', {
      area: 'psi',
      opcao: demoLocalChoice,
      segmento: demoNichoChoice,
    })
    setDemoOpen(false)
    router.push(`${PSI_DEMO_CLIENTE_BASE_PATH}?nicho=${encodeURIComponent(demoNichoChoice)}`)
  }, [demoLocalChoice, demoNichoChoice, router])

  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('psi_landing_escolha_inicial', { opcao: choice.analyticsValue, area: 'psi' })
      try {
        sessionStorage.setItem('ylada_psi_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Psicologia</p>
            ...
            <div className="pointer-events-auto w-full max-w-md max-h-[min(90dvh,640px)] overflow-y-auto overscroll-y-contain bg-white rounded-2xl p-6 estetica-safe-modal-bottom shadow-2xl border border-gray-200 animate-fade-in-up estetica-touch supports-[height:100svh]:max-h-[min(90svh,640px)]">
              {demoPhase === 'local' ? (
                ...
                <>
                  ...
                  <p className="mt-2 text-sm text-gray-600 leading-relaxed">
                    Cada opção abre um fluxo curto só para demonstração (reflexão, sem diagnóstico).
                  </p>
                  <div className="mt-5 flex flex-col gap-2">
                    {PSI_DEMO_CLIENTE_NICHOS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
                ...
                <EntradaSocraticaDemoIntroPanel
                  quemRespondeNoExemplo="quem te procura"
