...
/**
 * Entrada pública /pt/psicanalise — fluxo progressivo.
 * Landing minimal anterior: /pt/psicanalisev2.
 */
...
import {
  ...
  STORAGE_KEY_PSICANALISE_CONTINUAR_TOUR,
  PSICANALISE_LANDING_STEP_APOS_DEMO,
} from '@/lib/psicanalise-demo-context'
import { PSICANALISE_DEMO_CLIENTE_NICHOS } from '@/lib/psicanalise-demo-cliente-data'

const TOTAL_STEPS = 12
const CADASTRO_HREF = '/pt/cadastro?area=psicanalise'
...
const STEPS: StepBody[] = [
  {
    ...
    choices: [
      { label: 'Perguntas de preço ou frequência antes de contexto', analyticsValue: 'preco' },
      { label: 'Conversas que não viram sessão ou continuidade', analyticsValue: 'conversas' },
      { label: 'Pessoas com disposição real de começar um processo', analyticsValue: 'prontos' },
    ],
  },
  {
    lines: [
      'Se você atende como psicanalista…',
      '',
      'provavelmente isso já aconteceu com você:',
      '',
      'Você posta…',
      'mas quase não vira sessão de análise',
      '',
      'Às vezes até chama gente…',
      'mas não vira retorno ou regularidade',
      '',
      'E quando pergunta preço, frequência ou formato…',
      'a pessoa some',
      '',
      'Isso cansa.',
    ],
  ...
  },
  {
    lines: [
      'E não é você.',
      '',
      'O problema não é o seu trabalho no setting analítico.',
      '',
      'É que a maioria chega sem entender o que busca na análise, nem o que espera do primeiro contato.',
      '',
      'E quem não entende…',
      'compara preço.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Já com mais noção da inquietação e do tempo vivido',
      'Já com expectativa mais clara do processo',
      'E mais aberta pra fala e continuidade',
    ],
  ...
  },
  {
    lines: [
      ...
      '',
      'Perguntas que ajudam a pessoa a refletir',
      'sobre o que a inquieta, o contexto e o que busca no processo analítico',
    ],
    primary: { label: 'Continuar', action: 'next' },
  },
  {
  ...
  },
  {
    lines: [
      ...
      '',
      '“Quero atrair primeiras conversas sobre início de análise e dúvidas de quem nunca esteve no divã”',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'ela vai clareando o que sente e o que busca',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'E começa a enxergar coisas que antes não via.',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'menos conversa que morre na primeira mensagem',
      'menos curioso pedindo valor de sessão sem contexto',
      'mais clareza quando você abre o WhatsApp',
      'mais chance de conversa exploratória ou primeira sessão com alinhamento',
      '',
  ...
  },
  {
    lines: [
      ...
      '',
      'Precisa fazer quem busca análise chegar melhor.',
      '',
...
export default function PsicanaliseEntradaSocraticaContent() {
  ...
  const isPsicanaliseRoot =
    pathname === '/pt/psicanalise' || pathname.startsWith('/pt/psicanalise?')
  ...
  useEffect(() => {
    if (loading || !isPsicanaliseRoot) return
    if (user) {
      router.replace('/pt/psicanalise/home')
    }
  }, [loading, user, router, isPsicanaliseRoot])
  ...
  useEffect(() => {
    if (!isPsicanaliseRoot) return
    try {
      if (sessionStorage.getItem(STORAGE_KEY_PSICANALISE_CONTINUAR_TOUR) === '1') {
        sessionStorage.removeItem(STORAGE_KEY_PSICANALISE_CONTINUAR_TOUR)
        setStep(PSICANALISE_LANDING_STEP_APOS_DEMO)
      }
    } catch {
  ...
  const pickDemoLocal = useCallback((value: string) => {
    trackEvent('psicanalise_demo_local', { area: 'psicanalise', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_PSICANALISE_DEMO_LOCAL, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const pickDemoNicho = useCallback((value: string) => {
    trackEvent('psicanalise_demo_nicho', { area: 'psicanalise', opcao: value })
    try {
      sessionStorage.setItem(STORAGE_KEY_PSICANALISE_DEMO_NICHO, value)
    } catch {
      /* storage indisponível */
    }
  ...
  const startDemoQuiz = useCallback(() => {
    ...
    trackEvent('psicanalise_demo_inicio', {
      area: 'psicanalise',
    ...
    setDemoOpen(false)
    router.push(
      `${PSICANALISE_DEMO_CLIENTE_BASE_PATH}?nicho=${encodeURIComponent(demoNichoChoice)}`
    )
  }, [demoLocalChoice, demoNichoChoice, router])

  const onChoicePick = useCallback(
    (choice: ChoiceItem) => {
      trackEvent('psicanalise_landing_escolha_inicial', {
        opcao: choice.analyticsValue,
        area: 'psicanalise',
      })
      try {
        sessionStorage.setItem('ylada_psicanalise_entrada_opcao', choice.analyticsValue)
      } catch {
        /* storage indisponível */
      }
      goNext()
  ...
  const showAuthLoading = loading && isPsicanaliseRoot && !authTimeout
  if (showAuthLoading) {
    return (
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-4 sm:px-6 py-6 max-w-lg mx-auto w-full estetica-safe-main-bottom">
        ...
        >
          <div className="space-y-6 sm:space-y-8">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-400">Psicanálise</p>
            ...
            <div className="pointer-events-auto w-full max-w-md max-h-[min(90dvh,640px)] overflow-y-auto overscroll-y-contain bg-white rounded-2xl p-6 estetica-safe-modal-bottom shadow-2xl border border-gray-200 animate-fade-in-up estetica-touch supports-[height:100svh]:max-h-[min(90svh,640px)]">
              {demoPhase === 'local' ? (
                ...
                <>
                  ...
                  <p className="mt-2 text-sm text-gray-600 leading-relaxed">
                    Fluxo curto só para demonstração. Não substitui sessão nem setting analítico real.
                  </p>
                  <div className="mt-5 flex flex-col gap-2">
                    {PSICANALISE_DEMO_CLIENTE_NICHOS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
                ...
                <>
                  ...
                  <p className="mt-3 text-sm text-gray-700 leading-relaxed">
                    Responda como quem busca análise responderia. O restante você vê na sequência.
                  </p>
