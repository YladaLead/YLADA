...
/**
 * Demo: fluxo no lugar de quem busca psicanálise.
 * Rota: /pt/psicanalise/exemplo-cliente?nicho=inicio_processo
 */
...
import {
  PSICANALISE_DEMO_CLIENTE_NICHOS,
  getPsicanaliseDemoClienteConfig,
  type PsicanaliseDemoNicho,
} from '@/lib/psicanalise-demo-cliente-data'
import {
...
export default function PsicanaliseDemoClienteContent() {
  ...
  const [nichoAtivo, setNichoAtivo] = useState<PsicanaliseDemoNicho | null>(configFromUrl?.value ?? null)
  const [etapa, setEtapa] = useState(0)
  const [finalizado, setFinalizado] = useState(false)

  const cfg = useMemo(
    () => (nichoAtivo ? getPsicanaliseDemoClienteConfig(nichoAtivo) : null),
    [nichoAtivo]
  )
  ...
  const escolherNicho = useCallback(
    (v: PsicanaliseDemoNicho) => {
      try {
        sessionStorage.setItem(STORAGE_KEY_PSICANALISE_DEMO_NICHO, v)
      } catch {
        /* ok */
      }
      setNichoAtivo(v)
      setEtapa(0)
      setFinalizado(false)
      router.replace(`/pt/psicanalise/exemplo-cliente?nicho=${v}`, { scroll: false })
    },
    [router]
  )
  ...
  const avancar = useCallback(() => {
    ...
    if (etapa < total - 1) {
      ...
      setFinalizado(true)
      trackEvent('psicanalise_demo_cliente_concluiu', { area: 'psicanalise', opcao: cfg.value })
    }
  ...
  const continuarTour = useCallback(() => {
    try {
      sessionStorage.setItem(STORAGE_KEY_PSICANALISE_CONTINUAR_TOUR, '1')
    } catch {
      /* ok */
    }
    router.push('/pt/psicanalise')
  }, [router])
      ...
      <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full estetica-safe-main-bottom">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">
            Exemplo · quem busca análise
          </p>
          <h1 className="text-xl font-bold text-gray-900 mb-2">Escolha o foco do exemplo</h1>
          <p className="text-sm text-gray-600 mb-6 leading-relaxed">
            Cada opção abre um fluxo de demonstração com perguntas reflexivas. Não substitui sessão nem contrato de
            análise.
          </p>
          <div className="flex flex-col gap-2">
            {PSICANALISE_DEMO_CLIENTE_NICHOS.map((n) => (
              <button
                key={n.value}
                type="button"
                onClick={() => escolherNicho(n.value as PsicanaliseDemoNicho)}
          ...
          <Link href="/pt/psicanalise" className="mt-8 block text-center text-sm text-gray-500 hover:text-gray-800">
            ← Voltar
          </Link>
        </main>
      </div>
  ...
  if (nichoParam && !cfg) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-white">
        ...
        <Link href="/pt/psicanalise/exemplo-cliente" className="text-blue-600 font-medium text-sm">
          Ver opções de nicho
        </Link>
      </div>
    )
        ...
        <main className="flex-1 px-4 py-8 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/90 p-5 mb-6">
            ...
            <p className="text-sm text-emerald-900/95 leading-relaxed mb-3">
              ...
              <span className="font-semibold">não é análise, interpretação clínica nem contrato de setting</span>.
            </p>
            <p className="text-sm text-emerald-900/95 leading-relaxed">
              A ideia é <span className="font-semibold">organizar o que traz</span>, ganhar{' '}
              <span className="font-semibold">clareza</span> para o primeiro contato e chegar no Zap com mais contexto
              sobre o processo.
            </p>
          </div>
          ...
          <div
            ...
            role="group"
            aria-label="Modelo do botão de WhatsApp no link de psicanálise"
          >
            <div
          ...
          <p className="text-sm text-gray-700 leading-relaxed mb-4">
            ...
            <span className="font-semibold">contexto</span> para a conversa exploratória ou primeira sessão, com menos
            troca vaga só sobre <span className="font-semibold">preço ou “como funciona o divã”</span> sem história.
          </p>
          <p className="text-sm text-gray-700 leading-relaxed mb-8">
            O setting analítico continua sendo construído na clínica: o link ajuda a{' '}
            <span className="font-semibold">filtrar curiosidade</span> e <span className="font-semibold">preparar</span>{' '}
            quem tem disposição real para o processo.
          </p>
    ...
    <div className="min-h-[100dvh] bg-white text-gray-900 flex flex-col estetica-touch supports-[height:100svh]:min-h-[100svh]">
      ...
      <main className="flex-1 px-4 py-6 max-w-lg mx-auto w-full flex flex-col estetica-safe-main-bottom">
        <p className="text-xs font-semibold uppercase tracking-wider text-sky-700 bg-sky-50 border border-sky-100 rounded-lg px-3 py-2 mb-4 text-center leading-snug">
          Fluxo como quem busca análise veria · {cfg.label}
        </p>
