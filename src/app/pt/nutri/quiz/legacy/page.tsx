...
/** Quiz de carreira (autossegmentação, e-mail, resultado). O matriz YLADA está em /pt/nutri. */
...
const RESULTADO_POR_GRUPO: Record<Grupo, { titulo: string; subtitulo: string; bullets: string[]; esperanca: string }> = {
  recem_formada: {
    ...
    subtitulo:
      'O risco não é errar — é passar anos postando e se esforçando sem construir base. Com estrutura desde o início, você evita anos de tentativa e erro.',
    bullets: ['Falta clareza de posicionamento', 'Dúvida na hora de postar', 'Rotina ainda não organizada'],
  ...
  agenda_instavel: {
    ...
    subtitulo:
      'Pelas suas respostas, você posta com boa intenção mas sem um caminho claro do post até a consulta. Isso gera meses cheios e meses vazios.',
    bullets: ['Posts não conduzem para conversa', 'Dependência de indicação', 'Falta previsibilidade na agenda'],
  ...
  sobrecarregada: {
    ...
    subtitulo:
      'Trabalhar muito e decidir pouco gera cansaço e sensação de estar sempre correndo. O que falta é direção, não mais horas.',
    bullets: ['Rotina sem método', 'Improviso no que postar', 'Sensação de apagar incêndio'],
  ...
  financeiro_travado: {
    ...
    subtitulo:
      'Quando o posicionamento não sustenta o preço, a nutri trabalha muito e cresce pouco. Isso é perda silenciosa.',
    bullets: ['Preço abaixo do que deveria', 'Conteúdo que informa mas não valoriza', 'Falta clareza de posicionamento premium'],
  ...
  confusa: {
    ...
    subtitulo:
      'Fazer tudo sozinha, sem clareza do próximo passo, gera ansiedade ou estagnação. O que falta é um mapa, não mais esforço.',
    bullets: ['Falta clareza do próximo passo', 'Sensação de estar parada ou perdida', 'Muitas coisas ao mesmo tempo'],
    esperanca: 'Existe um próximo passo possível. O primeiro é ter clareza do que mudar e por onde começar.',
  },
}

export default function QuizNutriLegacyPage() {
      ...
      <main className="container mx-auto px-4 py-8 max-w-xl mx-auto">
        ...
        {step === 'result' && grupo && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-gray-600">Isso não é culpa sua. Nutricionistas não aprendem isso na faculdade.</p>
            <Link
