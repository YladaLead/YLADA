...
function NutriAgendaContent() {
  ...
  useEffect(() => {
    ...
    const carregarClientes = async () => {
      try {
        ...
        const response = await fetch('/api/nutri/clientes?limit=100&order=asc', {
          credentials: 'include'
        })
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      const response = await fetch(`/api/nutri/appointments/${consultaId}`, {
        method: 'PUT',
        headers: {
              ...
              return (
                ...
                >
                  {consultasSlot.length === 0 && onCelulaVaziaClick && (
                    <div className="absolute inset-0 w-full h-full opacity-0 group-hover:opacity-100 hover:bg-blue-50 transition-opacity flex items-center justify-center text-blue-600 text-xs font-medium pointer-events-none">
      ...
      <div className="grid grid-cols-7">
        {dias.map((dia, idx) => {
          ...
          return (
            ...
            >
              <div className={`text-sm font-medium mb-1 ${ehHoje ? 'text-blue-700' : 'text-gray-900'}`}>
                {dia.getDate()}
              </div>
              {onDiaClick && (
                <button
                  onClick={() => onDiaClick(dia)}
                  className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 p-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 transition-opacity"
            ...
            <button
              onClick={() => {
                if (typeof consulta.clients === 'object' && consulta.clients) {
                  router.push(`/pt/nutri/clientes/${consulta.clients.id}`)
                }
                onClose()
              }}
