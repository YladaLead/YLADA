...
export default function EditarPortalNutri() {
  return <EditarPortalNutriContent />
}

function EditarPortalNutriContent() {
  const { user } = useAuth()
  ...
  const carregarPortal = async () => {
    try {
      ...
      const response = await fetch(`/api/nutri/portals?id=${portalId}`, {
        credentials: 'include'
      })
      ...
      if (portal) {
        ...
        const toolsResponse = await fetch(`/api/nutri/portals/${portalId}/tools`, {
          credentials: 'include'
        })
  ...
  const verificarSlug = async (slug: string) => {
    ...
    try {
      const response = await fetch(`/api/nutri/portals/check-slug?slug=${encodeURIComponent(slug)}&excludeId=${portalId}`, {
        credentials: 'include'
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <NutriNavBar showTitle={true} title="Editar Portal de Bem-Estar" />
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-500">
                    {typeof window !== 'undefined' ? window.location.hostname : 'ylada.app'}/pt/nutri/
                    {userSlug ? (
                      <span className="text-blue-600 font-semibold">{userSlug}</span>
                    ) : (
                ...
                {checkingSlug ? (
                  ...
                  <p className="text-xs text-blue-600 mt-1">✓ Disponível</p>
                ) : slugAvailable === false ? (
                                  ...
                                  if (value.length >= 3) {
                                    ...
                                    try {
                                      const response = await fetch(
                                        `/api/nutri/check-short-code?code=${encodeURIComponent(value)}&type=portal&excludeId=${portalId}`
                                      )
                                      const data = await response.json()
