...
export default function RespostasFormulario() {
  return <RespostasFormularioContent />
}
...
function RespostasFormularioContent() {
  ...
  useEffect(() => {
    ...
    const carregarDados = async () => {
      try {
        ...
        // Carregar formulário
        const formResponse = await fetch(`/api/nutri/formularios/${formId}`, {
          credentials: 'include'
        })
  ...
  const carregarRespostas = async () => {
    try {
      ...
      const response = await fetch(`/api/nutri/formularios/${formId}/respostas?${params.toString()}`, {
        credentials: 'include'
      })
  ...
  const formatarResposta = (field: any, resposta: any) => {
    ...
    switch (field.type) {
      ...
      case 'time':
        return resposta
      case 'file':
        return resposta ? <a href={resposta} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Ver arquivo</a> : '-'
      default:
  ...
  const visualizarResposta = async (responseId: string) => {
    try {
      const response = await fetch(`/api/nutri/formularios/${formId}/respostas/${responseId}`, {
        credentials: 'include'
      })
