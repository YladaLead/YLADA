...
export default function AdminYladaUsoWellnessV1Page() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch('/api/admin/ylada/uso-wellness-v1?limit=500', { credentials: 'include' })
  ...
  const csvFilename = useMemo(() => {
    ...
    return `ylada-uso-wellness-v1-${d}.csv`
  }, [])

  const downloadCsv = useCallback(() => {
    const headers = ['id', 'data_hora', 'optional_noel', 'answers_json']
    const lines = [headers.join(';')]
    for (const r of rows) {
      const json = JSON.stringify(r.answers ?? {})
      const cols = [r.id, r.created_at, String(r.optional_noel ?? ''), json].map((c) =>
        `"${String(c).replace(/"/g, '""')}"`
      )
      ...
      <div className="min-h-screen bg-gray-50">
        <header className="bg-white border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 py-4">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mt-2">Pesquisa Wellness v1</h1>
            <p className="text-sm text-gray-600 mt-1 max-w-3xl">
              Respostas de <code className="text-xs bg-gray-100 px-1 rounded">/uso-wellness-v1</code>. Tabela
              Supabase:{' '}
              <code className="text-xs bg-gray-100 px-1 rounded">ylada_uso_wellness_v1_responses</code>. Em
              localhost os dados vão para o projeto configurado no <code className="text-xs bg-gray-100 px-1 rounded">.env.local</code>.
            </p>
          </div>
        </header>
        ...
        <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          {!loading && !error && total === 0 && (
            <p className="text-gray-600">
              Nenhuma resposta ainda. Se o envio falhou na página pública, rode o SQL da migration 299 no Supabase e
              confira o terminal do <code className="text-xs bg-gray-100 px-1 rounded">next dev</code> para o erro
              detalhado.
            </p>
          )}
          ...
          <section className="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
            ...
            <div className="divide-y divide-gray-100">
              {rows.map((r) => {
                const a = r.answers || {}
                return (
                  <details key={r.id} className="group px-4 py-3 hover:bg-emerald-50/30">
                    <summary className="cursor-pointer list-none flex flex-wrap gap-x-4 gap-y-1 text-sm">
                      ...
                      <span className="text-gray-800">
                        Barreira: <strong>{truncCell(a.main_barrier, 40)}</strong>
                      </span>
                      <span className="text-gray-700">
                        Uso/sem.: {truncCell(a.weekly_usage, 24)} · Barreira Noel:{' '}
                        <strong>{truncCell(a.noel_barrier, 28)}</strong>
                      </span>
                    </summary>
