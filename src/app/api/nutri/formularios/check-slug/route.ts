...
// Verificar disponibilidade de slug para formulários
export async function GET(request: NextRequest) {
  try {
    ...
    const palavrasReservadas = ['portal', 'ferramenta', 'ferramentas', 'formulario', 'formularios', 'home', 'configuracao', 'configuracoes', 'perfil', 'admin', 'api', 'pt', 'c', 'coach', 'nutri', 'wellness', 'nutra', 'quiz', 'quizzes']
    if (palavrasReservadas.includes(slug.toLowerCase())) {
      return NextResponse.json({
        slug,
        available: false,
        message: `"${slug}" é uma palavra reservada e não pode ser usada. Escolha outro nome.`
      })
    }
