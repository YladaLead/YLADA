...
export async function GET(request: NextRequest) {
  try {
    ...
    cleanExpiredCache()
    const cacheKey = `dashboard_coach_${authenticatedUserId}`
    const cached = dashboardCache.get(cacheKey)
    
    if (cached && (Date.now() - cached.timestamp) < CACHE_TTL) {
      const age = Math.round((Date.now() - cached.timestamp) / 1000)
      console.log(`⚡ Dashboard Coach API: ${Date.now() - startTime}ms (cache, idade: ${age}s)`)
    ...
    console.log(`⚡ Dashboard Coach API: ${duration}ms`)
    ...
    console.error('❌ Erro técnico ao buscar dados do dashboard Coach:', {
      error,
