      ...
      (portals || []).map(async (portal: any) => {
        const { data: tools } = await supabaseAdmin
          ...
          .select(`
            ...
            tool_id,
            redirect_to_tool_id,
            user_templates:coach_user_templates!tool_id (
              id,
              title,
              slug,
              emoji,
        ...
        return {
          ...portal,
          portal_tools: tools || [] // coach_portal_tools
        }
      })
    )
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verificar se o slug já existe PARA ESTE USUÁRIO (e profession='coach')
    ...
    if (body.generate_short_url) {
      if (body.custom_short_code) {
        ...
        const [existingInPortals, existingInQuizzes, existingInTemplates] = await Promise.all([
          supabaseAdmin.from('coach_portals').select('id').eq('short_code', customCode).limit(1),
        ...
        while (!codeData && attempts < 10) {
          ...
          if (!codeError && generatedCode) {
            ...
            const [checkPortals, checkQuizzes, checkTemplates] = await Promise.all([
              supabaseAdmin.from('coach_portals').select('id').eq('short_code', generatedCode).limit(1),
              supabaseAdmin.from('quizzes').select('id').eq('short_code', generatedCode).limit(1),
              supabaseAdmin.from('coach_user_templates').select('id').eq('short_code', generatedCode).limit(1),
            ])
    ...
    if (body.remove_short_code === true) {
      ...
      if (!existingPortal?.short_code) {
        ...
        if (body.custom_short_code) {
          ...
          const [existingInPortals, existingInQuizzes, existingInTemplates] = await Promise.all([
            supabaseAdmin.from('coach_portals').select('id').eq('short_code', customCode).neq('id', id).limit(1),
            supabaseAdmin.from('quizzes').select('id').eq('short_code', customCode).limit(1),
            supabaseAdmin.from('coach_user_templates').select('id').eq('short_code', customCode).limit(1),
          ])
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Deletar portal (CASCADE vai deletar coach_portal_tools automaticamente)
