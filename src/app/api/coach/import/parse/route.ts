...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isExcel) {
      ...
      try {
        ...
        if (excelError.message?.includes('corrupt') || excelError.message?.includes('invalid')) {
          return NextResponse.json(
            { error: 'Arquivo Excel corrompido ou formato inválido' },
            { status: 400 }
          )
        }
        throw excelError
    ...
    let errorMessage = 'Erro interno do servidor ao processar arquivo'
    ...
    if (error.code === 'ENOENT') {
      ...
      if (error.message.includes('corrupted') || error.message.includes('invalid') || error.message.includes('corrupt')) {
        errorMessage = 'Arquivo corrompido ou formato inválido'
      } else {
