...
import { fetchResetConfig, upsertResetConfig } from '@/lib/pro-lideres-reset'
...
export async function GET(request: NextRequest) {
  ...
  const config = await fetchResetConfig(ctx.tenant.id)
...
export async function POST(request: NextRequest) {
  ...
  if ('headline' in body) {
    ...
    if (h.length > 200) return NextResponse.json({ error: 'Título muito longo (máx 200 chars)' }, { status: 422 })
  ...
  if ('description' in body) {
    ...
    if (d.length > 500) return NextResponse.json({ error: 'Descrição muito longa (máx 500 chars)' }, { status: 422 })
    patch.description = d || undefined
  }

  const result = await upsertResetConfig(ctx.tenant.id, patch)
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: 500 })

  const updated = await fetchResetConfig(ctx.tenant.id)
