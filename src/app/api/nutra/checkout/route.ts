...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log('📥 Checkout Nutra request recebido')
    ...
    const authResult = await requireApiAuth(request, ['nutra', 'admin'])
    if (authResult instanceof NextResponse) {
      console.log('ℹ️ Checkout Nutra sem autenticação - usando e-mail fornecido')
    ...
    console.log(`✅ Checkout Nutra criado em ${checkoutDuration}ms: ${checkout.gateway} - ${checkout.sessionId}`)
    ...
    console.error('❌ Erro ao criar checkout Nutra:', error)
    return NextResponse.json(
      {
