...
export async function PATCH(request: NextRequest, context: Ctx) {
  ...
  const contentToCheck = (patch.content as Record<string, unknown> | undefined) ?? (existing.content as Record<string, unknown>)
...
export async function DELETE(request: NextRequest, context: Ctx) {
  ...
  const { error } = await supabaseAdmin.from('pro_lideres_consultancy_materials').delete().eq('id', id)
  if (error) {
