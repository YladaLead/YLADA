...
import {
  buildProEsteticaCorporalOnboardingUrl,
  corporalOnboardingExpiresAtDefault,
  generateCorporalOnboardingToken,
  isValidCorporalOnboardingEmail,
  normalizeCorporalOnboardingEmail,
...
export async function POST(request: NextRequest) {
  ...
  const invitedEmail = normalizeCorporalOnboardingEmail(body.email ?? '')

  if (professionalName.length < 2) {
    return NextResponse.json({ error: 'Nome inválido.' }, { status: 400 })
  }
  if (!isValidCorporalOnboardingEmail(invitedEmail)) {
    return NextResponse.json({ error: 'E-mail inválido.' }, { status: 400 })
  }

  const token = generateCorporalOnboardingToken()
  const expiresAt = corporalOnboardingExpiresAtDefault().toISOString()
  ...
  return NextResponse.json({
    item: data,
    onboarding_url: buildProEsteticaCorporalOnboardingUrl(requestOrigin(request), token),
  })
}
