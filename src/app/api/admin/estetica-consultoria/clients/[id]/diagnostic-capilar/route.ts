...
import { ensureDiagnosticoCapilarGlobalMaterialId } from '@/lib/estetica-consultoria-global-forms'

type Ctx = { params: Promise<{ id: string }> }

/** Painel: material global capilar + links desta clínica + respostas filtradas. */
export async function GET(request: NextRequest, context: Ctx) {
  ...
  if (seg !== 'capilar' && seg !== 'ambos') {
    return NextResponse.json({ error: 'Diagnóstico capilar só para segmento Capilar ou Ambos.' }, { status: 400 })
  }

  let materialId: string
  try {
    materialId = await ensureDiagnosticoCapilarGlobalMaterialId(supabaseAdmin)
  } catch (e) {
...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  if (!recipientEmailRaw || !EMAIL_RE.test(recipientEmailRaw)) {
    return NextResponse.json(
      {
        error:
          'Defina um e-mail válido da clínica em «Dados administrativos» antes de gerar o link. O convite de confirmação será enviado para esse e-mail.',
      },
      { status: 400 }
    )
  }
