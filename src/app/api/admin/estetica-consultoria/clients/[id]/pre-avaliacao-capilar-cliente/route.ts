...
import { ensurePreAvaliacaoCapilarClienteGlobalMaterialId } from '@/lib/estetica-consultoria-global-forms'

type Ctx = { params: Promise<{ id: string }> }

/** Pré-avaliação para cliente final: material global + links por clínica (sem entrada pública). */
export async function GET(request: NextRequest, context: Ctx) {
  ...
  if (seg !== 'capilar' && seg !== 'ambos') {
    return NextResponse.json(
      { error: 'Pré-avaliação cliente capilar só para segmento Capilar ou Ambos.' },
      { status: 400 }
    )
  }
  ...
  try {
    materialId = await ensurePreAvaliacaoCapilarClienteGlobalMaterialId(supabaseAdmin)
  } catch (e) {
...
export async function POST(request: NextRequest, context: Ctx) {
  ...
  const seg = String(cli.segment ?? '')
  if (seg !== 'capilar' && seg !== 'ambos') {
    return NextResponse.json({ error: 'Segmento inválido para esta pré-avaliação.' }, { status: 400 })
  }
