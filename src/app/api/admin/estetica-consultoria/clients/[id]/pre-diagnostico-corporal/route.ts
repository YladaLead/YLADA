...
/** Painel: material global pré-corporal + links + respostas desta clínica (sem gate de e-mail). */
export async function GET(request: NextRequest, context: Ctx) {
  ...
  if (seg !== 'corporal' && seg !== 'ambos') {
    return NextResponse.json({ error: 'Pré-diagnóstico corporal só para segmento Corporal ou Ambos.' }, { status: 400 })
  }

  let materialId: string
  try {
    materialId = await ensurePreDiagnosticoCorporalGlobalMaterialId(supabaseAdmin)
  } catch (e) {
