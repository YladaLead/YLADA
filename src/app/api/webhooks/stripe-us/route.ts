...
async function handleSubscriptionUpdated(
  ...
  // 🚨 CANCELAR ASSINATURAS MENSAIS quando assinatura anual é criada
  // Verificar se é uma nova assinatura (created) ou se está sendo atualizada para anual
  if (planType === 'annual' && status === 'active') {
    ...
    // Se não existe ou se está mudando de mensal para anual, cancelar mensais
    if (!existing || existing.plan_type !== 'annual') {
      await cancelMonthlySubscriptions(userId, area)
    }
  }
