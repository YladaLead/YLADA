...
import {
  ...
  PRO_ESTETICA_CAPILAR_NOEL_PATH,
} from '@/config/pro-estetica-capilar-menu'
import { PRO_ESTETICA_CAPILAR_NOEL_FOCUS_PARAM } from '@/config/pro-estetica-capilar-noel-focus'

const NOEL = PRO_ESTETICA_CAPILAR_NOEL_PATH

/** Home do painel — mesma hierarquia visual que Pro Estética Corporal (`ProEsteticaCorporalResolverHome`), copy em PT-BR. */
export default function ProEsteticaCapilarResolverHome() {
  return (
    <div className="mx-auto max-w-lg space-y-8 pb-4 lg:max-w-2xl">
      <header className="space-y-2">
        <p className="text-sm font-semibold text-blue-600">Pro Estética Capilar</p>
      ...
      <ul className="space-y-4">
        <li>
          <article className="overflow-hidden rounded-2xl border border-violet-200 bg-gradient-to-b from-violet-50/90 to-white p-4 shadow-sm sm:p-5">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Ideias e legenda no seu contexto (queda, cronograma, couro cabeludo) — sem ficar olhando para a tela em branco.
            </p>
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <Link
                href={`${NOEL}?${PRO_ESTETICA_CAPILAR_NOEL_FOCUS_PARAM}=atrair`}
            ...
            <p className="mt-3 text-xs text-gray-500">
              Também:{' '}
              <Link
                href={`${PRO_ESTETICA_CAPILAR_BASE_PATH}/biblioteca-links?tab=prontos`}
        ...
        <li>
          <article className="overflow-hidden rounded-2xl border border-sky-200 bg-gradient-to-b from-sky-50/90 to-white p-4 shadow-sm sm:p-5">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <Link
                href={`${NOEL}?${PRO_ESTETICA_CAPILAR_NOEL_FOCUS_PARAM}=responder`}
            ...
            <p className="mt-3 text-xs text-gray-500">
              Também:{' '}
              <Link
                href={`${PRO_ESTETICA_CAPILAR_BASE_PATH}/scripts`}
        ...
        <li>
          <article className="overflow-hidden rounded-2xl border border-emerald-200 bg-gradient-to-b from-emerald-50/90 to-white p-4 shadow-sm sm:p-5">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <Link
                href={`${NOEL}?${PRO_ESTETICA_CAPILAR_NOEL_FOCUS_PARAM}=reativar`}
            ...
            <p className="mt-3 text-xs text-gray-500">
              Também:{' '}
              <Link
                href={`${PRO_ESTETICA_CAPILAR_BASE_PATH}/retencao`}
