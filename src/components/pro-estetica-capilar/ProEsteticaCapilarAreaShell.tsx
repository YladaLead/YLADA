...
import ProEsteticaCapilarMobileNav from './ProEsteticaCapilarMobileNav'
import ProEsteticaCapilarCyberLinkAnalyticsBar from './ProEsteticaCapilarCyberLinkAnalyticsBar'
import { PRO_ESTETICA_CAPILAR_INICIO_PATH } from '@/config/pro-estetica-capilar-menu'
...
const PAINEL_HOME = PRO_ESTETICA_CAPILAR_INICIO_PATH
const PERFIL = '/pro-estetica-capilar/painel/perfil'
  ...
  return (
    <ProLideresPainelProvider value={painelContext}>
      <div className="flex min-h-screen min-h-[100dvh] bg-gray-50">
        <ProEsteticaCapilarSidebar isMobileOpen={mobileMenuOpen} onMobileClose={() => setMobileMenuOpen(false)} />
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {devStubPanel && !previewWithoutLogin && (
            ...
            >
              ...
              <code className="rounded bg-amber-100/80 px-1">PRO_ESTETICA_CAPILAR_DEV_OPEN_PAINEL=false</code>.
            </div>
          )}
          <header className="sticky top-0 z-30 flex min-h-14 items-center justify-between gap-2 border-b border-gray-200 bg-white px-3 sm:gap-3 sm:px-4 lg:px-6">
            <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
              ...
              >
                <Image
                  src={YLADA_LOGO}
                  alt="YLADA Pro Estética Capilar"
                  width={90}
                  height={28}
          ...
          <ProEsteticaCapilarCyberLinkAnalyticsBar previewWithoutLogin={previewWithoutLogin} />
          <main className="flex-1 p-4 pb-24 sm:p-5 lg:pb-6 lg:p-6">{children}</main>
          <ProEsteticaCapilarMobileNav />
        </div>
      </div>
