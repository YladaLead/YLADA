...
const PAINEL_PREFIX = '/pro-estetica-capilar/painel'
const PERFIL_PATH = `${PAINEL_PREFIX}/perfil`

interface ProEsteticaCapilarSidebarProps {
...
export default function ProEsteticaCapilarSidebar({
  isMobileOpen = false,
  onMobileClose,
}: ProEsteticaCapilarSidebarProps) {
  ...
  const filteredMenu = useMemo(() => {
    return PRO_ESTETICA_CAPILAR_MENU_GROUPS.map((g) => ({
      ...g,
      items: g.items.filter((item) => isLeaderWorkspace || !item.leaderOnly),
    })).filter((g) => g.items.length > 0)
  }, [isLeaderWorkspace])

  const itemHref = useCallback((item: ProEsteticaCapilarMenuItem) => proEsteticaCapilarItemHref(item.path), [])

  const itemIsActive = useCallback(
    (item: ProEsteticaCapilarMenuItem) => {
  ...
  const renderItemLink = (item: ProEsteticaCapilarMenuItem) => {
    const href = itemHref(item)
      ...
      <nav className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="shrink-0 border-t border-gray-200">
          <div className="px-3 pb-3 pt-2">
            {previewWithoutLogin ? (
              <Link
                href={`/pro-estetica-capilar/entrar?next=${encodeURIComponent(pathname || PRO_ESTETICA_CAPILAR_BASE_PATH)}`}
                onClick={onMobileClose}
