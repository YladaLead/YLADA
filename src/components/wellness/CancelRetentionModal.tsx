...
const RETENTION_STRATEGY: Record<CancelReason, RetentionOffer> = {
  ...
  'no_value': {
    type: 'show_feature',
    message: 'Entendo. Em 90% dos casos, o valor aparece quando a pessoa usa os links. Quer testar criar um link agora antes de sair?',
...
export default function CancelRetentionModal({
  ...
  const handleReasonSelect = async (reason: CancelReason) => {
    ...
    try {
      ...
      const response = await fetch('/api/wellness/subscription/cancel-attempt', {
        method: 'POST',
  ...
  const handleAcceptRetention = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/wellness/subscription/accept-retention', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
      ...
      if (retentionOffer.type === 'guided_tour') {
        // Redirecionar para NOEL
        window.location.href = '/pt/wellness/noel'
      } else if (retentionOffer.type === 'show_feature') {
        // Redirecionar para links
        window.location.href = '/pt/wellness/links'
      } else {
        // Para extend_trial e pause_subscription, mostrar mensagem e recarregar
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/wellness/subscription/confirm-cancel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
      ...
      try {
        const verifyResponse = await fetch('/api/wellness/subscription', {
          credentials: 'include'
        })
        ...
        if (isCanceled) {
          ...
          setTimeout(async () => {
            const recheckResponse = await fetch('/api/wellness/subscription', {
              credentials: 'include'
            })
            ...
            <div className="space-y-3">
              {(['no_time', 'didnt_understand', 'no_value', 'forgot_trial', 'too_expensive', 'found_alternative'] as CancelReason[]).map((reason) => (
                <button
                  ...
                  disabled={processing}
                  className="w-full text-left px-5 py-3.5 border-2 border-gray-200 rounded-xl hover:bg-green-50 hover:border-green-300 hover:shadow-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed group"
                >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {retentionOffer?.actionButton && (
                <button
                  ...
                  disabled={processing}
                  className="flex-1 px-5 py-3.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-semibold hover:from-green-700 hover:to-emerald-700 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                >
                  {processing ? (
