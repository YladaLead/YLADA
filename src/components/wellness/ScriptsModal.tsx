...
export function ScriptsModal({
  ...
  const config = ferramentaSlug 
    ? getScriptsPorSlug(ferramentaSlug)
    : getScriptsPorNome(ferramentaNome)
  ...
  const { listaQuente, listaFria, indicacao } = getScriptsPorTipo(config)

  const tabs: { id: TabTipo; label: string; icon: string; scripts: ScriptFerramenta[] }[] = [
  ...
  const copiarScript = async (script: ScriptFerramenta) => {
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-700 text-white p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-green-100 text-sm">{ferramentaNome}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
          <p className="text-xs text-gray-600">
            ...
            {tabAtiva === 'lista_fria' && '❄️ Para desconhecidos, redes sociais, grupos ou pessoas que não te conhecem'}
            {tabAtiva === 'indicacao' && '🔄 Para pedir indicações depois que a pessoa já usou a ferramenta'}
          </p>
        </div>
