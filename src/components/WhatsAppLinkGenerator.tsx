'use client'

import { useState } from 'react'
import { Copy, Check } from 'lucide-react'
import { getToolMessage } from '@/lib/tool-messages'

interface WhatsAppLinkGeneratorProps {
  toolName: string
  userName: string
  projectName: string
  className?: string
}

export default function WhatsAppLinkGenerator({ 
  toolName, 
  userName, 
  projectName, 
  className = '' 
}: WhatsAppLinkGeneratorProps) {
  const [copied, setCopied] = useState(false)
  
  // Obter mensagem personalizada baseada na ferramenta
  const toolMessage = getToolMessage(toolName)
  const whatsappMessage = toolMessage.shortMessage
  
  // URL do link personalizado
  const linkUrl = `https://www.herbalead.com/${userName}/${projectName}`
  
  // Mensagem completa para WhatsApp
  const fullWhatsAppMessage = `${whatsappMessage}\n\n${linkUrl}`
  
  // URL do WhatsApp com a mensagem
  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(fullWhatsAppMessage)}`
  
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText(linkUrl)
      setCopied(true)
      setTimeout(() => setCopied(false), 2000)
    } catch (error) {
      console.error('Erro ao copiar link:', error)
    }
  }
  
  const handleCopyMessage = async () => {
    try {
      await navigator.clipboard.writeText(fullWhatsAppMessage)
      setCopied(true)
      setTimeout(() => setCopied(false), 2000)
    } catch (error) {
      console.error('Erro ao copiar mensagem:', error)
    }
  }
  
  const handleOpenWhatsApp = () => {
    window.open(whatsappUrl, '_blank')
  }

  return (
    <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
      <h3 className="text-lg font-semibold text-gray-900 mb-4">
        📱 Compartilhar no WhatsApp
      </h3>
      
      {/* Preview da mensagem */}
      <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
        <p className="text-sm text-green-800 font-medium mb-2">
          Mensagem que será enviada:
        </p>
        <p className="text-sm text-green-700">
          {whatsappMessage}
        </p>
        <p className="text-xs text-green-600 mt-2 font-mono">
          {linkUrl}
        </p>
      </div>
      
      {/* Botões de ação */}
      <div className="space-y-3">
        <button
          onClick={handleOpenWhatsApp}
          className="w-full px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-semibold flex items-center justify-center"
        >
          <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488"/>
          </svg>
          Abrir WhatsApp
        </button>
        
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={handleCopyMessage}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center justify-center"
          >
            {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
            <span className="ml-2">Copiar Mensagem</span>
          </button>
          
          <button
            onClick={handleCopyLink}
            className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors font-medium flex items-center justify-center"
          >
            {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
            <span className="ml-2">Copiar Link</span>
          </button>
        </div>
      </div>
      
      {/* Informações técnicas */}
      <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
        <p className="text-xs text-gray-600 mb-2">
          <strong>💡 Dica:</strong> A imagem será exibida automaticamente no WhatsApp com:
        </p>
        <ul className="text-xs text-gray-600 space-y-1">
          <li>• <strong>Título:</strong> {toolMessage.title}</li>
          <li>• <strong>Descrição:</strong> {toolMessage.description}</li>
          <li>• <strong>Imagem:</strong> Otimizada para redes sociais (1200x630px)</li>
        </ul>
      </div>
    </div>
  )
}
