...
const HUB_MEUS = `${PRO_ESTETICA_CORPORAL_BASE_PATH}/biblioteca-links?tab=meus`
...
function filterProEsteticaCorporalLinks(rows: ApiLink[]): ApiLink[] {
...
export default function ProEsteticaCorporalLinksAnaliseClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(filterProEsteticaCorporalLinks(json.data))
    } catch {
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 text-center text-sm text-gray-600">
          ...
          <Link href={`${PRO_ESTETICA_CORPORAL_BASE_PATH}/biblioteca-links?tab=prontos`} className="font-medium text-sky-700 underline">
            Biblioteca
          </Link>{' '}
          ou no Noel.
        </div>
