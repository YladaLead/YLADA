...
import {
  PRO_ESTETICA_RITMO_PLACEBO_METRICS,
  shouldShowProEsteticaRitmoPlacebo,
} from '@/config/pro-estetica-corporal-ritmo-demo'
...
export default function ProEsteticaCorporalResultadosClient() {
  const { user } = useAuth()
  const placebo = shouldShowProEsteticaRitmoPlacebo(user?.email ?? null)
  const m = PRO_ESTETICA_RITMO_PLACEBO_METRICS
