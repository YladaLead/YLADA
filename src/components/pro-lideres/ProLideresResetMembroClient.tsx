...
export default function ProLideresResetMembroClient() {
  ...
  const copyQr = async () => {
    if (!data?.resetUrl) return
    const ok = await copyYladaLinkQrAsPng(data.resetUrl)
    if (ok) {
      setCopiedQr(true)
      ...
      {data?.resetUrl ? (
        ...
        >
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-900">Seu link Reset Metabólico</h2>
              <p className="text-sm text-[#5A8D2A]">{data.headline ?? 'Reset Metabólico'}</p>
            </div>
          </div>

          <div className="mb-4 rounded-xl bg-white px-4 py-3 shadow-inner">
            <p className="break-all text-sm font-mono text-gray-700">{data.resetUrl}</p>
          </div>
          ...
          {!data.hasWhatsapp ? (
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
              ...
              <p className="mt-1 text-xs text-amber-700">
                Sem WhatsApp, o botão de encomendar sacola não funciona. Cadastre em{' '}
          ...
          {!data.videoConfigured ? (
            <div className="mt-3 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800">
              ℹ️ O vídeo ainda não foi configurado pelo líder. A página já funciona — o botão de WhatsApp aparece
              normalmente.
            </div>
          ) : null}
        </div>
        ...
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:p-6">
          ...
          <p className="text-sm text-amber-800">
            Cadastre o seu <strong>slug de divulgação</strong> no perfil para gerar o link personalizado.{' '}
            <Link href="/pro-lideres/membro/perfil" className="font-semibold underline">
              Ir para o perfil →
            </Link>
          </p>
