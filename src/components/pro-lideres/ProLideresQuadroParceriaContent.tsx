...
// ---------------------------------------------------------------------------
// Scripts de abordagem Pro Líderes
// ---------------------------------------------------------------------------
...
Não tem custo, não interfere na rotina — só agrega valor.`,
  },
  {
    ...
    texto: `Os líderes que mais crescem são os que criam diferenciais além do produto.
...
Isso posiciona você como alguém moderno, que usa tecnologia para cuidar das pessoas de verdade.`,
  },
  {
    ...
    texto: `(Ótimo para quem busca oportunidade de negócio)

Quem chega até aqui já está em busca de mudança — seja na saúde, na energia ou na vida financeira.
...
// Sem reticências, sem jargão técnico, estimulam a pessoa a interagir
// ---------------------------------------------------------------------------
...
function QuadroCard({ item, qrSize }: { item: ItemQuadro; qrSize: number }) {
  const url = resolveAbsoluteProLideresCatalogPublicUrl(item.publicUrl)
  const motivacional = textoMotivacional(item.label)
  const ogUrl = getOGImageUrl(item.presetFluxoId ?? null, 'wellness')
...
// ---------------------------------------------------------------------------
// Defaults do cabeçalho (editáveis pelo líder)
// ---------------------------------------------------------------------------
...
function QuadroFolha({
  ...
  return (
    ...
    >
      {/* Header editável — gradiente azul → violeta, mais chamativo */}
...
// ---------------------------------------------------------------------------
export function ProLideresQuadroParceriaContent() {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        ...
        // Só links de saúde (sales) — excluir recrutamento/ganhos
        const soSaude = (data.catalog ?? []).filter(
          (item: ProLideresCatalogItem) => item.catalogCategory === 'sales'
        )
        setCatalog(soSaude)
      } catch (e) {
        console.error(e)
  ...
  const adicionar = (item: ProLideresCatalogItem) => {
    ...
    setSelecionados((prev) => [
      ...prev,
      {
        ...
        presetFluxoId: item.presetFluxoId ?? null,
      },
    ])
  }
  ...
  const salvarPdf = async () => {
    ...
    try {
      ...
      doc.save('quadro-parceria-pro-lideres.pdf')
    } catch (e) {
      console.error(e)
    } finally {
      setExportando(false)
  ...
  return (
    <div className="min-h-screen bg-slate-50 print:bg-white">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:max-w-md flex-shrink-0">
            ...
            <h2 className="text-lg font-semibold text-slate-800 mb-1">
              {loading ? 'Carregando links…' : 'Escolha os links de saúde'}
            </h2>
            {!loading && catalog.length > 0 && (
              <p className="text-xs text-slate-500 mb-3">
                Apenas ferramentas de saúde e bem-estar. Links de negócio/recrutamento não aparecem neste quadro.
              </p>
            )}
            ...
            {loading ? (
              ...
              <p className="text-sm text-slate-500">
                Nenhum link encontrado. Adicione links em{' '}
                <a href="/pro-lideres/painel/catalogo" className="text-blue-600 underline">
                  Meus links
                </a>
                .
              </p>
      ...
      <div id="quadro-impressao-pl" className="hidden print:block print:max-w-none w-full">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media print {
          ...
          #quadro-impressao-pl, #quadro-impressao-pl * { visibility: visible; }
          #quadro-impressao-pl { position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; padding: 0 !important; margin: 0 !important; box-sizing: border-box; }
          #quadro-impressao-pl .quadro-pagina-impressao { page-break-after: always; width: 100%; height: 277mm; min-height: 277mm; max-height: 277mm; padding: 0; margin: 0; box-sizing: border-box; display: flex; flex-direction: column; }
          #quadro-impressao-pl .quadro-pagina-impressao:last-child { page-break-after: auto; }
          #quadro-impressao-pl .quadro-pagina-conteudo { width: 100%; height: 100%; min-height: 0; flex: 1; display: flex; flex-direction: column; border: none; border-radius: 0; box-shadow: none; }
        }
      `}} />
    </div>
  )
