...
export default function ProLideresVideoShareMembroClient({
  kind,
  pageLabel,
}: {
  kind: VideoShareKind
  ...
  const load = useCallback(() => {
    setLoading(true)
    fetch(`/api/pro-lideres/video-share/${kind}/meu-link`)
      .then((r) => r.json())
  ...
  const copyQr = async () => {
    if (!data?.shareUrl) return
    const ok = await copyYladaLinkQrAsPng(data.shareUrl)
    if (ok) {
      setCopiedQr(true)
      ...
      {data?.shareUrl ? (
        ...
        >
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-900">Seu link de {pageLabel}</h2>
              <p className="text-sm text-[#5A8D2A]">{data.headline ?? pageLabel}</p>
            </div>
          </div>

          <div className="mb-4 rounded-xl bg-white px-4 py-3 shadow-inner">
            <p className="break-all text-sm font-mono text-gray-700">{data.shareUrl}</p>
          </div>
        ...
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:p-6">
          ...
          {!data?.shareSlug ? (
            <p className="text-sm text-amber-800">
              Para ter um link personalizado de {pageLabel}, você precisa cadastrar o seu{' '}
            ...
            <p className="text-sm text-amber-800">Algo deu errado ao gerar o seu link. Fale com o líder.</p>
          )}
        </div>
      )}
    </div>
